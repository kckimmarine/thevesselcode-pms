...
async function boot() {
  if (window.PMS_ISO_needsSetup?.()) {
    $('#setup-screen').classList.remove('hidden');
    $('#login-screen').classList.add('hidden');
    $('#setup-form').addEventListener('submit', (e) => {
      e.preventDefault();
      const key = new FormData(e.target).get('anonKey');
      if (!window.PMS_ISO_saveAnonKey(key)) {
        $('#setup-error').textContent = '키를 입력하세요.';
        return;
      }
