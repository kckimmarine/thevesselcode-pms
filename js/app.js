...
const TVC_App = (function () {
    ...
    async function applyDefectClearedSchedules() {
        ...
        for (const dc of cleared) {
            const code = defectEffectiveJobCode(dc);
            const job = (dc.maintenance_job_id && jobById.get(dc.maintenance_job_id))
                || (code ? jobByCode.get(code) : null);
            if (!job) continue;
    ...
    function menuXferDefaultChannelHint(user) {
        ...
        if (ctx === 'station') return 'Default transfer: offline ZIP → Master (email/USB).';
        if (ctx === 'master') return 'Default transfer: offline ZIP ↔ HQ (email). Online sync available when configured.';
        if (ctx === 'hq') return 'Default transfer: offline ZIP import (email fallback). Online sync available when configured.';
    ...
    function renderMenuXferModal() {
        ...
        if (step === 'mode') {
            ...
            const exportNote = ctx === 'station'
                ? 'Monthly Report exports your department delta to Master Hub.'
                : ctx === 'master'
            ...
            const importHint = ctx === 'station'
                ? 'Import HQ feedback ZIP received by email (HQ → Ship).'
                : ctx === 'master'
                    ? 'Import Engine/Deck station ZIP or HQ feedback ZIP. Station packages merge into Master Hub.'
                    : ctx === 'hq'
    ...
    async function openMenuXferMenu() {
        const f = state.user
            ...
            : {};
        if (!f.showDataXfer && !f.showExportHq && !f.showImportHq) {
    ...
    async function menuXferConfirmMonthlyExport() {
        ...
        const destLabel = menuXferExportTargetLabel(target);
        if (!await TVC_Dialog.confirm({ message: `Export Monthly Report to ${destLabel}?` })) return;
        closeMenuXferMenu();
        try {
    ...
    async function menuXferExportMonthly(target) {
        ...
        await TVC_Dialog.alert(`${TVC_RBAC.getDeptLabel(dept)} Monthly Report exported.`);
    }
        ...
        if (name.endsWith('.zip')) {
            ...
            if (jsonFile) {
                ...
                if (dir === 'STATION_TO_HUB') {
                    ...
                    throw new Error('Station export ZIP must be imported on Master login (Captain Hub).');
                }
    ...
    async function openOrigJobModify() {
        if (!canEditOriginalPlanItems()) await TVC_Dialog.alert(origPlanEditDeniedMessage());
        if (isOrigJobInlineEditing()) return saveOrigJobInlineEdit();
        if (!state.selectedJobId) await TVC_Dialog.alert('Select a job row to modify.');
        const job = state.idx?.jobById.get(state.selectedJobId);
        if (!job) await TVC_Dialog.alert('Job item not found.');
    ...
    async function deleteOrigJob() {
        if (!canEditOriginalPlanItems()) await TVC_Dialog.alert(origPlanEditDeniedMessage());
        if (!state.selectedJobId) await TVC_Dialog.alert('Select a job row to delete.');
        const job = state.idx?.jobById.get(state.selectedJobId);
        if (!job) return;
    ...
    async function saveBatchReport() {
        ...
        if (!entries.length) await TVC_Dialog.alert('No jobs to save.');
    ...
    function jobWorkHistoryEntries(jobId) {
        ...
        return workHistoryEntriesRaw().filter(e => {
            if (e.source === 'defect') {
                const dc = e.defect;
                return dc && (
                    dc.maintenance_job_id === job.id
                    || dc.job_code === job.job_code
                    || defectEffectiveJobCode(dc) === job.job_code
                );
            }
            const item = e.item;
    ...
    function isHistDefectRowConfirmable(entry) {
        ...
        return TVC_DefectReport.isDefectReportConfirmable(dc);
    }

    function isHistRowCheckable(entry) {
        if (isHistDefectEntry(entry)) {
            if (TVC_RBAC.isHqAccount(state.user)) {
                return isHistDefectRowConfirmable(entry) || isHistDefectRowHqApprovable(entry);
            }
    ...
    async function histReportApproval() {
        ...
        for (const id of reportIds) {
            ...
            if (!hasReported) await TVC_Dialog.alert(`${rep.job_code}: Cannot confirm in this status.`);
            const dept = reportDept(rep);
            if (!TVC_RBAC.canConfirmDepartment(user, dept)) {
                await TVC_Dialog.alert(`Other department (${dept || '?'}) report cannot be confirmed: ${rep.job_code}`);
            }
        }
        ...
        for (const id of reportIds) {
            try {
                ...
                const opts = (rep?.work_type === 'POSTPONE' && postponeRequiresCompanyApproval(rep))
                    ...
                    : {};
                await TVC_Transaction.approveReport(user, id, '', opts);
                ok++;
            } catch (e) {
    ...
    function renderWorkProcedureModal() {
        ...
        if (state._wpTab === 'procedure') {
            ...
            const histRows = histEntries.length ? histEntries.map(entry => {
                ...
                <td class="hist-type ${esc(m.title ? 'hist-type-defect' : '')}"><span class="hist-type-mark">${esc(m.letter)}</span></td>
                <td>${esc(fileNo)}</td>
                <td>${esc(dt || '—')}</td>
    ...
    function buildWorkReportPrintBody() {
        ...
        const canConfirmNow = !!rep && TVC_RBAC.isReportedStatus(rep.status, rep.is_locked)
            && TVC_RBAC.canConfirmDepartment(state.user, job.department);
        ...
        const page1Html = `<div class="wr-print-page">
            <h2 class="wr-print-page-title">Page 1 — ${esc(page1Label)}</h2>
            <div class="wr-print-doc tone-${esc(state._wrTab)}"><div class="wr-page">${page1Body}</div></div>
        </div>`;
        let page2Html = '';
        if (state._wrTab === 'repair') {
            ...
            page2Html = TVC_SpareMenu.buildWrSparePage2PrintHtml(state, state._wrUsedParts, meta);
        }
        return { title, html: page1Html + page2Html };
    }
    ...
    function renderWrReportFooter(opts = {}) {
        ...
        return `
            ...
            ${fld('', renderWrAttachmentBlock('company', { canUpload: canEditCompanyAttach && !ro }), 'wr-maint-span-all wr-maint-grid-gap')}
        `;
    }
        ...
        return `<div class="wr-maint-form">
            ...
            <section class="wr-maint-card wr-maint-body wr-file-no-anchor">
                ...
                <div class="wr-maint-grid wr-maint-grid-2 wr-maint-grid-gap">
                    ...
                    ${fld('Postpone Date', `<input type="date" class="tvc-date-input" data-wf="postponeDate" placeholder="YYYY-MM-DD" value="${esc(wf('postponeDate'))}"${ro ? ' disabled' : ''}>`, 'wr-postpone-date')}
                </div>
    ...
    function renderWrApprovalHtml(opts = {}) {
        ...
        return `<section class="wr-maint-card wr-maint-approval">
            <div class="wr-maint-approval-item${canConfirmNow ? ' is-active' : ''}">
                <label class="wr-maint-chk"><input type="checkbox" id="wrConfirmedBy" ${isRepConfirmed ? 'checked' : ''} ${canConfirmNow ? '' : 'disabled'} onchange="TVC_App.wrReportConfirmByToggle()"> Confirmed by</label>
    ...
    function renderWorkReportModal(opts = {}) {
        ...
        const isRepConfirmed = !!rep && TVC_RBAC.isConfirmedStatus(rep.status, rep.is_locked);
        const isRepApproved = !!rep && reportIsApproved(rep);
        const canConfirmNow = !!rep && TVC_RBAC.canConfirmDepartment(state.user, job.department)
            && (TVC_RBAC.isReportedStatus(rep.status, rep.is_locked)
                || (!ro && TVC_RBAC.isConfirmedStatus(rep.status, rep.is_locked) && !isRepApproved));
        const hqDirectApprove = !!rep && !isRepApproved && TVC_RBAC.canHqDirectApprove(state.user, rep);
        const canApproveNow = !!rep && !isRepApproved && TVC_RBAC.canApproveHqReport(state.user)
            && (isRepConfirmed || hqDirectApprove);
        const reportedByName = workReportReportedByName(rep);
    ...
    async function requestCloseWorkReport() {
        if (isNewUnsavedWorkReportSession()) {
            ...
            if (yes) resetAndCloseWorkReport();
            return;
        }
        closeWorkReport();
    }
        ...
        if (user && canApproveFromUi) {
            try {
                ...
                if (postponeRequiresCompanyApproval(rep) && !approvedPostponeDate) {
                    await TVC_Dialog.alert('Approved Enter Postpone Date.');
                }
    ...
    async function importPmsMasterExcel(file) {
        const user = TVC_Auth.getCurrentUser();
        if (!user || !canPmsMasterExcel()) await TVC_Dialog.alert(pmsMasterExcelDeniedMessage());
        if (!file) return;
    ...
    return {
        ...
        navReport, deleteWorkReport, printWorkReport, previewWorkReport, wrReportConfirmByToggle, closeWorkReport, requestCloseWorkReport,
