...
const TVC_SpareMenu = (function () {
    ...
    const SPARE_EDIT_IDENTITY_W = Math.round((SPARE_MAIN_DWG_WIDTH + SPARE_MAIN_PART_NO_WIDTH + SPARE_MAIN_ITEMS_WIDTH) / 3);
    const SPARE_MAIN_COL_WIDTHS = [32, 92, 62, SPARE_MAIN_DWG_WIDTH, SPARE_MAIN_PART_NO_WIDTH, SPARE_MAIN_ITEMS_WIDTH, 44, 56, 56, 56, 56, 56];
    ...
    const SPARE_EDIT_COLGROUP = `<colgroup>${[
        32, 92, 62,
        SPARE_EDIT_IDENTITY_W, SPARE_EDIT_IDENTITY_W, SPARE_EDIT_IDENTITY_W,
        44, 56, 56, 56, 56, 56,
    ].map(w => `<col style="width:${w}px">`).join('')}</colgroup>`;
    ...
    const CONSUME_LOG_COLGROUP = `<colgroup>
        ...
        <col class="cl-col-sort1"><col class="cl-col-sort2"><col class="cl-col-reported"><col class="cl-col-status">
        <col class="cl-col-total">
    </colgroup>`;
    ...
    function consumeLogLinkedDefect(log, st) {
        if (!log) return null;
        const cases = st?.defectCases || [];
        if (log.defect_case_id) return cases.find(d => d.id === log.defect_case_id) || null;
        if (log.id) return cases.find(d => d.consume_log_id === log.id) || null;
        return null;
    }
    ...
    /** Page 1 FILE NO / VOY NO / PLACE — from linked Maintenance / Defect report when missing on log. */
    function resolveConsumeLogPage1Meta(log, st = getState()) {
        const out = {
            file_no: String(log?.file_no || '').trim(),
            voy_no: String(log?.voy_no || '').trim(),
            place: String(log?.place || '').trim(),
        };
        if (out.file_no && out.voy_no && out.place) return out;
        const src = String(log?.source || '').toLowerCase();
        if (src === 'work_report' || log?.work_report_id) {
            const rep = (st.reports || []).find(r =>
                (log.work_report_id && r.id === log.work_report_id)
                || (log.id && r.consume_log_id === log.id)
            );
            if (rep) {
                const f = rep.report_form || {};
                const itemForm = (rep.job_items || []).find(it => it.form)?.form || {};
                out.file_no = out.file_no || String(f.fileNo || itemForm.fileNo || '').trim();
                out.voy_no = out.voy_no || String(f.voyNo || itemForm.voyNo || '').trim();
                out.place = out.place || String(f.place || itemForm.place || '').trim();
            }
        } else if (src === 'defect_report' || src === 'defect' || log?.defect_case_id) {
            const dc = (st.defectCases || []).find(d =>
                (log.defect_case_id && d.id === log.defect_case_id)
                || (log.id && d.consume_log_id === log.id)
            );
            if (dc) {
                out.file_no = out.file_no || String(dc.file_no || '').trim();
                out.voy_no = out.voy_no || String(dc.voy_no || '').trim();
                out.place = out.place || String(dc.place || '').trim();
            }
        }
        return out;
    }
    ...
    const SPARE_ITEM_HIST_COL_WIDTHS = [92, 62, SPARE_MAIN_DWG_WIDTH, SPARE_MAIN_PART_NO_WIDTH, SPARE_MAIN_ITEMS_WIDTH, 44, 56, 56, 56];
    ...
    function sectionCodeFromPartNo(partNo) {
        ...
        const legacy = s.match(/^(\d{2}-\d{3})-/);
        if (legacy) return legacy[1];
        return '';
    }

    function sectionCodeFromSpare(s) {
        const c = s ? canon(s) : null;
        if (!c) return '';
        const fromCode = sectionCodeFromPartNo(c.inventoryNumbering || '');
        if (fromCode) return fromCode;
        const dwg = String(c.drawingPartNo || c.dwgNo || '').trim();
        const m = dwg.match(/(?:sheet\s*no\.?\s*)?(\d{2}-\d{3})/i);
        if (m) return m[1];
        return sectionCodeFromPartNo(c.makerPartNo || c.part_no);
    }
    ...
    function assyNameFromSpare(s) {
        ...
        const secCode = sectionCodeFromSpare(s);
    ...
    function equipmentNamesForGroup(st, groupLabel) {
        ...
        const fromSpares = (st.spares || [])
            .map(canon)
            .filter(s => {
                const g = spareGroupLabel(s);
                if (isGeneratorEngineGroupLabel(lab)) return isGeneratorEngineGroupLabel(g);
                return normalizeGroupLabel(g) === normalizeGroupLabel(lab);
            })
    ...
    function findEquipmentGroupDef(st, groupLabel, equipmentName) {
        const lab = String(groupLabel || '').trim();
        ...
        return (st.groups || []).find(gr => matchLabel(gr) && String(gr.item_sort1 || '').trim() === item) || null;
    }

    function equipmentNoFromRow(gr) {
        if (!gr) return 0;
        const n = parseInt(String(gr.equipment_no ?? gr.equipmentNo ?? gr.sort_order ?? ''), 10);
        return Number.isFinite(n) && n > 0 ? Math.min(99, n) : 0;
    }

    function equipmentNoForName(st, groupLabel, equipmentName) {
        const item = String(equipmentName || '').trim();
        if (!item) return 0;
        const def = findEquipmentGroupDef(st, groupLabel, item);
        const fromDef = equipmentNoFromRow(def);
        if (fromDef) return fromDef;
        if (typeof TVC_SpareCode !== 'undefined') {
            const sp = (st.spares || []).map(canon).find(s => {
                const g = spareGroupLabel(s);
                return equipmentFromSpare(s) === item
                    && normalizeGroupLabel(g) === normalizeGroupLabel(groupLabel);
            });
    ...
    function equipmentNoFromSpare(st, spare, groupLabel) {
        ...
        if (!s) return 0;
        if (s.equipmentNo > 0) return s.equipmentNo;
        const eqName = equipmentFromSpare(s);
        if (eqName) {
            const fromName = equipmentNoForName(st, groupLabel || spareGroupLabel(s), eqName);
            if (fromName) return fromName;
        }
        if (typeof TVC_SpareCode !== 'undefined') {
            const fromCode = TVC_SpareCode.resolveEquipNo(s);
            if (fromCode) return fromCode;
        }
        return 0;
    }

    function nextEquipmentNoForGroup(st, groupLabel) {
        const lab = String(groupLabel || '').trim();
        if (!lab) return 1;
        const nums = equipmentDefsForGroup(st, lab).map(d => d.equipmentNo).filter(n => n > 0);
        if (!nums.length) return 1;
        return Math.min(99, Math.max(...nums) + 1);
    }

    function sparesForEquipment(st, groupLabel, equipmentName) {
        const lab = String(groupLabel || '').trim();
        const item = String(equipmentName || '').trim();
        if (!lab || !item) return [];
        return (st.spares || []).map(canon).filter(s => {
            const g = spareGroupLabel(s);
            const groupMatch = isGeneratorEngineGroupLabel(lab)
                ? isGeneratorEngineGroupLabel(g)
                : normalizeGroupLabel(g) === normalizeGroupLabel(lab);
            return groupMatch && equipmentFromSpare(s) === item;
        });
    }
    ...
    function equipmentDefsForGroup(st, groupLabel) {
        return equipmentNamesForGroup(st, groupLabel).map(name => {
            ...
            return {
                ...
                name,
                equipmentNo,
                maker: def?.maker || headerFieldText(row?.maker) || '',
                modelType: def?.modelType || headerFieldText(row?.model_type) || '',
                capacity: def?.capacity || headerFieldText(row?.capacity) || '',
                serialNo: def?.serialNo || headerFieldText(row?.serial_no) || '',
                spareCount: sparesForEquipment(st, groupLabel, name).length,
                hasHeader: !!(def?.edited || row?.header_edited),
            };
    ...
    function renderEquipmentFieldLabel(st, hasContent) {
        const canManage = hasContent && canEditGroupHeader(st);
        const btn = canManage
            ? `<button type="button" class="spare-gh-manage-btn" onclick="TVC_SpareMenu.openEquipmentListModal()" title="Add / edit / delete Equipment list">📋 List</button>`
            : '';
        return `<span class="spare-gh-label-row"><span class="spare-gh-label">Equipment</span>${btn}</span>`;
    }

    function renderHeaderEquipmentField(st, h, opts = {}) {
        const {
            hasContent = true,
            editMode = false,
    ...
    function enrichSpareHeaderFields(st, base, sampleSpare, opts = {}) {
        ...
        if (sample) {
            const secCode = sectionCodeFromSpare(sample);
    ...
    function resolveSpareHeaderFromSpare(st, spare) {
        ...
        if (equipment) {
            const equipDef = groupDefHeader(st, pmsGroupNo === '—' ? '' : pmsGroupNo, equipment);
            return {
        ...
        if (equipment) {
            ...
            if (equipDef?.edited) {
                return {
                    ...
                    maker: equipDef.maker,
                    serialNo: equipDef.serialNo,
                    criticalEquipment: equipDef.criticalEquipment || '',
                    assyName: '',
                    dwgNo: '',
                };
            }
    ...
    function renderSpareGroupHeaderFromResolved(h, opts = {}) {
        ...
        return `<section class="spare-group-header spare-group-header-in-list${hasContent ? '' : ' is-idle'}" aria-label="${esc(ariaLabel)}">
            <div class="spare-group-header-card">
                <div class="spare-gh-row spare-gh-row-primary spare-gh-row-primary-split">
                    <div class="spare-gh-field spare-gh-field-wide">
                        <span class="spare-gh-label">${pmsLabel}</span>
        ...
        return `<section class="spare-group-header is-editing is-group-editing" aria-label="Edit group header">
            <div class="spare-group-header-card">
                ...
                <div class="spare-gh-edit-actions">
                    <span class="spare-gh-edit-hint">${esc(equipHint)}</span>
    ...
    function suggestNextSpareCode(st, groupLabel, equipmentName) {
    ...
    function resolveAppendGroupHeader(st) {
        const blank = {
            pmsGroupNo: '', equipment: '', machineryName: '', modelType: '', capacity: '',
        ...
        const header = groupLabel
            ? enrichSpareHeaderFields(st, { ...blank, pmsGroupNo: groupLabel }, null)
            : { ...blank };
        return header;
    }

    function resolveAppendRowDraft(st, header) {
        const code = suggestNextSpareCode(st, header.pmsGroupNo || '', header.equipment || '');
        return {
            code,
            class: '',
            dwgNo: '',
            ...
            <div class="spare-item-edit-table-wrap">
                <table class="spare-data-table spare-item-edit-table">
                    ...
                    <tbody><tr class="spare-row-editing">
                        ...
                        <td class="c-item">${rowCellInput('sie_item', r.item)}</td>
    ...
    function modState(st) {
        ...
        st.spareModule = st.spareModule || {
            ...
            consumeLogListMode: false,
            consumeLogListEditing: false,
            ...
            reqListPeriodTo: '',
            reqListSearch: '',
            reqListFilterType: 'all',
            reqListFilterGroupKeys: [],
            reqListFilterOpen: false,
            consumeLogCheckedIds: {},
            selectedConsumeLogId: null,
            consumeLogPeriodFrom: '',
            consumeLogPeriodTo: '',
            consumeLogSearch: '',
            consumeLogFilterType: 'all',
            consumeLogFilterGroupKeys: [],
            inlineEditId: null,
            inlineDraft: null,
            ...
            equipmentListEditKey: null,
            equipmentListDraft: null,
        };
    ...
    function compareSpareByCode(a, b) {
        ...
        if (!ca) return 1;
        if (!cb) return -1;
        if (typeof TVC_SpareCode !== 'undefined') return TVC_SpareCode.compare(ca, cb);
        return ca.localeCompare(cb, undefined, { numeric: true, sensitivity: 'base' });
    }

    // 부품 코드(예: "01-001") 앞자리 → PMS Group 번호 (정수)
    function spareCodeGroupNo(s) {
        const code = String(spareNumbering(s) || '').trim();
        if (typeof TVC_SpareCode !== 'undefined') {
            const g = TVC_SpareCode.groupNoFromCode(code);
    ...
    function spareDwgNo(s, st) {
        const c = canon(s);
        const direct = c.dwgNo ?? c.dwg_no ?? c.drawing_no ?? c.drawingNo;
        if (direct) return headerFieldText(direct);
        const state = st || getState();
        const secCode = sectionCodeFromSpare(c);
    ...
    function applyReqWorkScrollLock(body) {
        ...
        const keepActive = new Set(['reqWorkHistBtn', 'reqHistListFilterBtn']);
        scroll.querySelectorAll('input, select, textarea, button').forEach(el => {
            if (keepActive.has(el.id)) return;
            // Requisition List hist popover stays interactive (Period / Search / Filter / checkboxes / Print / Preview)
            if (el.closest('#reqWorkHistPanel')) return;
        ...
        unlockReqWorkHistPanelControls();
    ...
    function wrSpareListEditable() {
        const m = modState(getState());
        return !!m.wrSpareOpen && !m.wrSpareReadonly;
    }
          ...
          <div class="plan-layout spare-layout${panelOpen ? ' panel-open' : ''}">
            ...
            <main class="panel spare-main">
              <div class="filter-bar orig-toolbar spare-item-toolbar">
                ...
                <button type="button" id="actSpareMasterExportBtn" class="btn btn-sm btn-excel hidden" onclick="TVC_App.exportSpareMasterExcel()" title="Export SPARE Master Excel — edit in Excel, then Master Import">📤 Master Export</button>
                <button type="button" id="actSpareMasterImportBtn" class="btn btn-sm hidden" onclick="TVC_App.triggerSpareMasterImport()" title="Import SPARE Master Excel after editing export file">📥 Master Import</button>
    ...
    function applySpareToolbarFlags(tb) {
        ...
        if (spareEx) {
            ...
            spareEx.title = canMaster
                ? 'Export Master Excel (Group · Equipment · Spare Parts) — edit file, then Master Import'
        ...
        if (spareIm) {
            ...
            spareIm.title = canMaster
                ? 'Import edited Master Excel (codes GG-EE-III, Equipment, ROB/Std)'
                : (typeof TVC_App?.spareMasterExcelDeniedMessage === 'function' ? TVC_App.spareMasterExcelDeniedMessage() : '');
        }
        const m = modState(getState());
    ...
    function rowHtml(s, focusedId, batchMap, ctx = 'main') {
        ...
        const focusClick = ctx === 'reqWork'
            ...
            : ctx === 'wrSpare'
                ? `TVC_SpareMenu.wrSpareFocusRow('${sid}', ${wrSpareListEditable() ? 'true' : 'false'})`
            : `${focusFn}('${sid}')`;
        ...
        const dblAttr = (ctx === 'reqWork' || ctx === 'receive' || ctx === 'consume' || ctx === 'wrSpare') ? '' : ` ondblclick="event.preventDefault();${dblFn}"`;
    ...
    /** Requisition Received 단계 — 입고 수량 기록 완료 */
    ...
    function reqVesselReceivedExportTabHint(req) {
        ...
        if (!reqVesselReceiveRecorded(req)) {
            return 'Complete received quantities in Spare Parts Requisition first.';
        }
    ...
    function resolveReqListGroupKey(req, st = getState()) {
        if (!req) return null;
        const direct = String(req.pms_group_key || req.spare_group_key || '').trim();
        ...
        for (const line of req.lines || []) {
            ...
            if (!gl) continue;
            if (st.idx?.groupNodes) {
                const node = st.idx.groupNodes.find(n =>
                    String(n.label || '').trim() === gl
                    || String(n.key || '').endsWith('|' + gl)
                );
                if (node) return node.key;
            }
            return `${s.department || st.department || ''}|${gl}`;
        }
        const pmsNo = String(req.pms_group_no || req.spare_group_label || '').trim();
    ...
    /** Open = not Received yet, or Received but still has O/S remaining. */
    function reqListIsOpen(req) {
        if (!req) return false;
        if (reqWorkflowPhase(req) !== REQ_LIST_PHASE.RECEIVED) return true;
        return reqListOutstandingData(req) > 0;
    }

    function getReqListFilters() {
        const m = modState(getState());
        return {
            groupKeys: [...(m.reqListFilterGroupKeys || [])],
            type: m.reqListFilterType || 'all',
            openOnly: !!m.reqListFilterOpen,
        };
    }

    function setReqListFilters(patch = {}) {
        const m = modState(getState());
        if (patch.groupKeys !== undefined) {
            m.reqListFilterGroupKeys = Array.isArray(patch.groupKeys) ? [...patch.groupKeys] : [];
        }
        if (patch.type !== undefined) {
            m.reqListFilterType = String(patch.type || 'all').toLowerCase() || 'all';
        }
        if (patch.openOnly !== undefined) {
            m.reqListFilterOpen = !!patch.openOnly;
        }
        if (typeof TVC_ListFilters !== 'undefined') TVC_ListFilters.syncBtn('reqList');
        refreshReqListUi();
    }

    function reqListMatchesTypeAndGroup(req, st) {
        const m = modState(st);
        const type = String(m.reqListFilterType || 'all').toLowerCase();
        const groupKeys = m.reqListFilterGroupKeys || [];
        if (type && type !== 'all') {
            if (String(reqListTypeKind(req) || '').toLowerCase() !== type) return false;
        }
        if (groupKeys.length) {
            const gk = resolveReqListGroupKey(req, st);
            if (!gk || !groupKeys.includes(gk)) return false;
        }
        if (m.reqListFilterOpen && !reqListIsOpen(req)) return false;
        return true;
    }
    ...
    function reqListFilterBase(reqs, st) {
        ...
        return (reqs || []).filter(req =>
            ...
            && reqListMatchesTypeAndGroup(req, st));
    }
    ...
    function reqListIsConfirmedForApprove(req) {
        // legacy helper — HQ Approve now uses Submitted only (reqListIsSubmittedForApprove)
        const flags = reqListApprovalFlags(req);
        if (!flags.isConfirmed || flags.isApproved) return false;
    ...
    function reqListIsSubmittedForApprove(req) {
        if (!req) return false;
        return reqWorkflowPhase(req) === REQ_LIST_PHASE.SUBMITTED
    ...
    function canConfirmRequisition(st, req) {
        ...
        // Confirm = vessel mode only; Reported enables, Confirmed+ disables
        if (isReqListHqUser(st)) return false;
        if (!reqListIsReportedForConfirm(req)) return false;
        const dept = req.department || st.department;
    ...
    function canApproveRequisition(st, req) {
        ...
        if (reqListApprovalFlags(req).isApproved) return false;
        // HQ Superintendent 작성분: Reported만 있어도 Approve 가능
    ...
    function activeReqListFilterPrefix() {
        const hist = document.getElementById('reqWorkHistPanel');
        if (_reqWorkHistOpen && hist && !hist.classList.contains('hidden')
            && hist.querySelector('#reqHistPeriodFrom')) return 'reqHist';
        return 'reqList';
    }

    function syncReqListFilterUi(st) {
        const m = modState(st);
        ['reqList', 'reqHist'].forEach(prefix => {
            ...
            const filterEl = document.getElementById(prefix + 'PeriodFilter');
            ...
            const clearBtn = searchEl?.parentElement?.querySelector('.search-clear-btn');
            if (clearBtn) clearBtn.classList.toggle('hidden', !m.reqListSearch);
        });
        if (typeof TVC_ListFilters !== 'undefined') TVC_ListFilters.syncBtn('reqList');
    }

    function renderReqListFiltersHtml(m, opts = {}) {
        const prefix = opts.prefix || 'reqList';
        const btnId = prefix === 'reqHist' ? 'reqHistListFilterBtn' : 'reqListFilterBtn';
        return `<div class="hist-toolbar hist-toolbar-filters spare-req-list-filters list-filter-stack">
                ...
                <div class="filter-bar list-filter-search-row">
                    <div class="search-field-wrap">
                        <input class="search-input" id="${prefix}Search" placeholder="Search REQ NO / PORT / STATUS…"
    ...
    function reqListSetPeriod() {
        const st = getState();
        const m = modState(st);
        const prefix = activeReqListFilterPrefix();
        m.reqListPeriodFrom = document.getElementById(prefix + 'PeriodFrom')?.value || '';
        m.reqListPeriodTo = document.getElementById(prefix + 'PeriodTo')?.value || '';
        refreshReqListUi();
    }
    ...
    function reqListSetSearch(v) {
        ...
        _reqListSearchT = setTimeout(() => refreshReqListUi(), 150);
    }

    function reqListClearSearch() {
        modState(getState()).reqListSearch = '';
        ['reqListSearch', 'reqHistSearch'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.value = '';
        });
        refreshReqListUi();
    ...
    async function buildConsumedLogListPrintBody() {
        ...
        if (m.consumeLogFilterType && m.consumeLogFilterType !== 'all') {
            filterParts.push(`Type: ${String(m.consumeLogFilterType).toUpperCase()}`);
        }
        if ((m.consumeLogFilterGroupKeys || []).length) {
            filterParts.push(`PMS Group: ${m.consumeLogFilterGroupKeys.length} selected`);
        }
        const head = `<tr>
        ...
        const rows = logs.map(log => {
            ...
            const page1 = resolveConsumeLogPage1Meta(log);
            return `<tr>
                <td class="num">${esc(consumeLogTypeMarker(log).letter)}</td>
                <td>${esc(page1.file_no || '—')}</td>
                <td>${esc(jobText || '—')}</td>
                <td>${esc(sort1Text || '—')}</td>
                <td>${esc(sort2Text || '—')}</td>
        ...
        const meta = await spareListPrintMeta('Consumption List');
        return `${meta}
            ${spareListPrintFilterNote(filterParts)}
            <p class="meta">${logs.length} item(s)</p>
            <table>${head}${rows || `<tr><td colspan="8">No consumed logs to print.</td></tr>`}</table>`;
    }
    ...
    async function buildRequisitionListPrintBody() {
        ...
        if (m.reqListFilterType && m.reqListFilterType !== 'all') {
            filterParts.push(`Type: ${String(m.reqListFilterType)}`);
        }
        if ((m.reqListFilterGroupKeys || []).length) {
            filterParts.push(`PMS Group: ${m.reqListFilterGroupKeys.length} selected`);
        }
    ...
    async function patchReqListUi() {
        ...
        body.querySelectorAll('.spare-req-list-count').forEach(el => { el.textContent = countLabel; });
        const tbody = body.querySelector('#spareReqListScroll .spare-req-list-body-table tbody');
        if (tbody) tbody.innerHTML = reqRows;
        ...
        const clearBtn = searchEl?.parentElement?.querySelector('.search-clear-btn');
        if (clearBtn) clearBtn.classList.toggle('hidden', !m.reqListSearch);
        const periodEl = document.getElementById('reqListPeriodFilter');
        if (periodEl) periodEl.classList.toggle('active', !!(m.reqListPeriodFrom || m.reqListPeriodTo));
        if (typeof TVC_ListFilters !== 'undefined') TVC_ListFilters.syncBtn('reqList');
    }

    function reqListCanSoftPatch() {
        const modal = document.getElementById('spareReqListModal');
        return !!(modal && !modal.classList.contains('hidden')
            && document.querySelector('#spareReqListScroll .spare-req-list-body-table tbody'));
    }

    async function refreshReqListUi(opts = {}) {
        const modal = document.getElementById('spareReqListModal');
        if (modal && !modal.classList.contains('hidden')) {
            if (!opts.full && reqListCanSoftPatch()) await patchReqListUi();
            else await renderReqListModal();
        }
        if (_reqWorkHistOpen) await refreshReqWorkHistList(opts.full ? { full: true } : {});
    }
    ...
    function syncReqListRowSelection(root) {
        ...
        scope.querySelectorAll('.spare-req-list-body-table tbody tr[data-req-id]').forEach(tr => {
            ...
            tr.classList.toggle('sr-req-sel', id === selId);
            const cb = tr.querySelector('input[type="checkbox"]');
            if (cb && !cb.disabled) cb.checked = reqListIsRowChecked(m, id);
        });
    }

    function reqListScrollSelectedIntoView() {
        const scroll = document.getElementById('reqWorkHistListScroll')
            || document.getElementById('spareReqListScroll');
        if (!scroll) return;
        const sel = scroll.querySelector('tr.sr-req-sel');
        if (sel) sel.scrollIntoView({ block: 'nearest' });
    }

    async function reqListMoveSelection(delta) {
        const { vesselId } = await vesselScope();
        const reqs = filterReqList(await TVC_Inventory.listRequisitions(vesselId), getState());
        if (!reqs.length) return;
        const m = modState(getState());
        let idx = reqs.findIndex(r => r.id === m.selectedReqId);
        if (idx < 0) idx = delta > 0 ? -1 : 0;
        const next = Math.max(0, Math.min(reqs.length - 1, idx + delta));
        await reqListSelectRow(reqs[next].id);
        requestAnimationFrame(reqListScrollSelectedIntoView);
    }

    function onReqListKeydown(e) {
        if (e.key !== 'ArrowUp' && e.key !== 'ArrowDown') return;
        const t = e.target;
        if (t && (t.tagName === 'INPUT' || t.tagName === 'TEXTAREA' || t.tagName === 'SELECT' || t.isContentEditable)) {
            if (t.closest('#reqWorkHistPanel') || t.closest('#spareReqListBody')) {
                if (t.classList.contains('search-input')
                    || t.classList.contains('act-period-input')
                    || t.classList.contains('tvc-date-input')) return;
            }
        }
        const histOpen = _reqWorkHistOpen && document.getElementById('reqWorkHistPanel')
            && !document.getElementById('reqWorkHistPanel').classList.contains('hidden');
        const standalone = document.getElementById('spareReqListModal')
            && !document.getElementById('spareReqListModal').classList.contains('hidden');
        if (!histOpen && !standalone) return;
        if (histOpen && !isRequisitionListWindow() && !standalone) {
            // New Requisition pick list — still allow arrow nav
        }
        e.preventDefault();
        e.stopPropagation();
        reqListMoveSelection(e.key === 'ArrowDown' ? 1 : -1);
    }

    function bindReqListKeyboard() {
        const workModal = document.getElementById('spareReqWorkModal');
        if (workModal && !workModal._reqListKeyBound) {
            workModal._reqListKeyBound = true;
            workModal.addEventListener('keydown', onReqListKeydown, true);
        }
        const standalone = document.getElementById('spareReqListModal');
        if (standalone && !standalone._reqListKeyBound) {
            standalone._reqListKeyBound = true;
            standalone.addEventListener('keydown', onReqListKeydown, true);
        }
    }
    ...
    async function reqListSelectRow(id) {
        ...
        const scroll = document.getElementById('reqWorkHistListScroll')
            || document.getElementById('spareReqListScroll');
        if (scroll && document.activeElement?.closest?.('.spare-req-list-filters') == null
    ...
    async function renderReqListModal() {
        ...
        const showConfirm = !isHq && !isAuthor;
        const showCloseOs = !isAuthor;
        const headToolbar = exportXfer
            ...
            ${showConfirm ? spareListToolbarBtn('Report Confirm', 'TVC_SpareMenu.reqListReportConfirm()', !canConfirm, 'btn-green') : ''}
    ...
    /* ── Consumption List window (consumeLogListMode) — mirrors Requisition List ── */

    function isConsumedLogWindow(m) {
        const s = m?.consumeLogListMode !== undefined ? m : modState(getState());
        return !!s.consumeOpen && !!s.consumeLogListMode;
    }

    function consumeLogHistPickBtnLabel(listMode) {
        return listMode ? 'Select File No.' : 'Consumption List';
    }

    function consumeReportWindowTitle(m) {
        const s = m?.consumeLogListEditing !== undefined ? m : modState(getState());
        return s.consumeLogListEditing ? 'Consumption Report (Draft)' : 'Consumption Report';
    }
    ...
    function syncConsumeReportWindowTitle() {
        const m = modState(getState());
        if (!isConsumedLogWindow(m)) return;
        const el = document.getElementById('consumeReportHeadTitle');
        if (el) el.textContent = consumeReportWindowTitle(m);
    }

    function syncSparePartsRequisitionWindowTitle() {
        const m = modState(getState());
        if (!isRequisitionListWindow(m) || !m.reqWorkOpen) return;
        const el = document.getElementById('reqWorkHeadTitle');
        if (el) el.textContent = sparePartsRequisitionWindowTitle(m);
    }

    function consumeLogListHasDisplayedLog(m) {
        const s = m?.consumeLogListMode !== undefined ? m : modState(getState());
        if (!isConsumedLogWindow(s)) return false;
        const draft = _consumeDraft;
        if (!draft) return false;
        if (draft.log_id) return true;
        const fileNo = String(draft.file_no || document.getElementById('consumeFileNo')?.value || '').trim();
        const hasLines = (draft.lines || []).length > 0;
        return !!(fileNo && hasLines);
    }

    function consumeLogFormLocked() {
        const m = modState(getState());
        if (m.consumePreview) return true;
        return isConsumedLogWindow(m) && !m.consumeLogListEditing;
    }

    function applyConsumeLogScrollLock(body) {
        const scroll = body?.querySelector?.('.spare-req-work-scroll');
        if (!scroll) return;
        const locked = consumeLogFormLocked();
        scroll.classList.toggle('is-req-locked', locked);
        const keepActive = new Set(['consumeLogHistBtn', 'consumeLogListFilterBtn']);
        scroll.querySelectorAll('input, select, textarea, button').forEach(el => {
            if (keepActive.has(el.id)) return;
            // Consumption List hist popover stays interactive (Period / Search / Filter / Confirm / Delete / Print / Preview)
            if (el.closest('#consumeLogHistPanel')) return;
            if (locked) el.setAttribute('disabled', 'disabled');
            else el.removeAttribute('disabled');
        });
        if (!locked) syncSpareToolbarUi();
    ...
    function syncConsumeLogRowSelection(root) {
        const scope = root || document;
        const m = modState(getState());
        const selId = m.selectedConsumeLogId;
        scope.querySelectorAll('.spare-consume-log-body-table tbody tr[data-consume-log-id]').forEach(tr => {
            const id = tr.getAttribute('data-consume-log-id');
            tr.classList.toggle('sr-req-sel', id === selId);
            const cb = tr.querySelector('input.spare-row-chk');
            if (cb) cb.checked = consumeLogIsRowChecked(m, id);
        });
        const logs = [...scope.querySelectorAll('.spare-consume-log-body-table tbody tr[data-consume-log-id]')]
            .map(tr => ({ id: tr.getAttribute('data-consume-log-id') }))
            .filter(l => l.id);
        updateConsumeLogHeadCheckAll(logs);
    }

    function consumeLogHistScrollSelectedIntoView() {
        const scroll = document.getElementById('consumeLogHistListScroll')
            || document.getElementById('spareConsumeLogScroll');
        if (!scroll) return;
        const sel = scroll.querySelector('tr.sr-req-sel');
        if (sel) sel.scrollIntoView({ block: 'nearest' });
    }

    async function consumeLogMoveSelection(delta) {
        const { vesselId } = await vesselScope();
        const logs = filterConsumeLogs(await TVC_Inventory.listConsumeLogs(vesselId), getState());
        if (!logs.length) return;
        const m = modState(getState());
        let idx = logs.findIndex(l => l.id === m.selectedConsumeLogId);
        if (idx < 0) idx = delta > 0 ? -1 : 0;
        const next = Math.max(0, Math.min(logs.length - 1, idx + delta));
        await consumeLogSelectRow(logs[next].id);
        requestAnimationFrame(consumeLogHistScrollSelectedIntoView);
    }

    function onConsumeLogHistKeydown(e) {
        if (e.key !== 'ArrowUp' && e.key !== 'ArrowDown') return;
        const t = e.target;
        if (t && (t.tagName === 'INPUT' || t.tagName === 'TEXTAREA' || t.tagName === 'SELECT' || t.isContentEditable)) {
            // Period / Search typing — do not steal arrows
            if (t.closest('#consumeLogHistPanel') || t.closest('#spareConsumeLogBody')) {
                if (t.classList.contains('search-input')
                    || t.classList.contains('act-period-input')
                    || t.classList.contains('tvc-date-input')) return;
            }
        }
        const histOpen = _consumeLogHistOpen && document.getElementById('consumeLogHistPanel')
            && !document.getElementById('consumeLogHistPanel').classList.contains('hidden');
        const standalone = document.getElementById('spareConsumeLogModal')
            && !document.getElementById('spareConsumeLogModal').classList.contains('hidden');
        if (!histOpen && !standalone) return;
        if (!isConsumedLogWindow() && !standalone) return;
        e.preventDefault();
        e.stopPropagation();
        consumeLogMoveSelection(e.key === 'ArrowDown' ? 1 : -1);
    }

    function bindConsumeLogHistKeyboard() {
        const modal = document.getElementById('spareConsumeModal');
        if (modal && !modal._consumeLogHistKeyBound) {
            modal._consumeLogHistKeyBound = true;
            modal.addEventListener('keydown', onConsumeLogHistKeydown, true);
        }
        const standalone = document.getElementById('spareConsumeLogModal');
        if (standalone && !standalone._consumeLogHistKeyBound) {
            standalone._consumeLogHistKeyBound = true;
            standalone.addEventListener('keydown', onConsumeLogHistKeydown, true);
        }
    }

    function syncConsumeLogListWindowHeadButtons() {
        const m = modState(getState());
        if (!isConsumedLogWindow(m)) return;
        const head = document.querySelector('#spareConsumeBody .spare-req-work-head');
        if (!head) return;
        const hasDisplayed = consumeLogListHasDisplayedLog(m);
        const modifyBtn = document.getElementById('consumeLogHeadModifyBtn');
        const newBtn = document.getElementById('consumeLogHeadNewBtn');
        const saveBtn = document.getElementById('consumeLogSaveBtn');
        const cancelBtn = document.getElementById('consumeLogCancelBtn');
        if (modifyBtn) modifyBtn.disabled = !hasDisplayed || m.consumeLogListEditing;
        if (newBtn) newBtn.disabled = m.consumeLogListEditing || !canCreateConsume(getState());
        if (saveBtn) saveBtn.disabled = !m.consumeLogListEditing;
        if (cancelBtn) cancelBtn.disabled = !m.consumeLogListEditing;
        setSpareListToolbarBtnDisabled(head, 'consumeLogPrintPreview', !hasDisplayed);
        setSpareListToolbarBtnDisabled(head, 'consumeLogWindowDocPreview', !hasDisplayed);
    ...
    function resolveConsumeLogGroupKey(log, st = getState()) {
        if (!log) return null;
        const direct = String(log.pms_group_key || log.spare_group_key || '').trim();
        if (direct) return direct;
    ...
    function getConsumeLogListFilters() {
        const m = modState(getState());
        return {
            groupKeys: [...(m.consumeLogFilterGroupKeys || [])],
            type: m.consumeLogFilterType || 'all',
        };
    }

    function setConsumeLogListFilters(patch = {}) {
        const m = modState(getState());
        if (patch.groupKeys !== undefined) {
            m.consumeLogFilterGroupKeys = Array.isArray(patch.groupKeys) ? [...patch.groupKeys] : [];
        }
        if (patch.type !== undefined) {
            m.consumeLogFilterType = String(patch.type || 'all').toLowerCase() || 'all';
        }
        if (typeof TVC_ListFilters !== 'undefined') TVC_ListFilters.syncBtn('consumeLog');
    ...
    function filterConsumeLogs(logs, st) {
        ...
        const type = String(m.consumeLogFilterType || 'all').toLowerCase();
        const groupKeys = m.consumeLogFilterGroupKeys || [];
        return (logs || []).filter(log => {
            if (!consumeLogDateInPeriod(log.made_on || log.consumed_date, from, to)) return false;
            if (!consumeLogMatchSearch(log, q)) return false;
            if (type && type !== 'all') {
                const letter = String(consumeLogTypeMarker(log).letter || '').toLowerCase();
                if (letter !== type) return false;
            }
            if (groupKeys.length) {
                const gk = resolveConsumeLogGroupKey(log, st);
                if (!gk || !groupKeys.includes(gk)) return false;
            }
            return true;
        });
    ...
    function consumeLogCanApprove(st, log) {
        if (!log) return false;
        const user = spareInventoryUser(st);
        if (!user || !TVC_RBAC.canApproveHqReport(user)) return false;
        const status = spareListStatus(log);
        const isConfirmed = status === SPARE_LIST_STATUS.CONFIRMED || status === SPARE_LIST_STATUS.APPROVED
            || !!(log?.confirmed_by || log?.confirmed_at);
        const isApproved = status === SPARE_LIST_STATUS.APPROVED || !!(log?.approved_by || log?.approved_at);
        if (isApproved) return false;
        // HQ Superintendent 작성분: Reported만 있어도 Approve 가능
        if (TVC_RBAC.canHqDirectApprove(user, log)) {
            return status !== SPARE_LIST_STATUS.DRAFT;
        }
        return isConfirmed;
    }

    function consumeLogCheckedConfirmableIds(m, allLogs, st) {
        const map = m.consumeLogCheckedIds || {};
        return Object.keys(map).filter(id => {
            if (!map[id]) return false;
            const log = allLogs.find(l => l.id === id);
            return consumeLogCanConfirm(st, log);
        });
    }

    function consumeLogCheckedApprovableIds(m, allLogs, st) {
        const map = m.consumeLogCheckedIds || {};
        return Object.keys(map).filter(id => {
            if (!map[id]) return false;
            const log = allLogs.find(l => l.id === id);
            return consumeLogCanApprove(st, log);
        });
    }

    function consumeLogAnyCheckedCanConfirm(m, allLogs, st) {
        return consumeLogCheckedConfirmableIds(m, allLogs, st).length > 0;
    }

    function consumeLogAnyCheckedCanApprove(m, allLogs, st) {
        return consumeLogCheckedApprovableIds(m, allLogs, st).length > 0;
    }
    ...
    function renderConsumeLogFiltersHtml(m) {
        return `<div class="hist-toolbar hist-toolbar-filters spare-consume-log-filters list-filter-stack">
                ...
                <div class="filter-bar list-filter-search-row">
                    <div class="search-field-wrap">
                        <input class="search-input" id="consumeLogSearch" placeholder="Search File No / JOB CODE / SORT / STATUS…"
    ...
    function consumeLogSetSearch(v) {
        ...
        _consumeLogSearchT = setTimeout(() => refreshConsumeLogListUi(), 150);
    }
    ...
    function consumeDraftFromLog(log) {
        ...
        const draft = {
            ...
            defect_case_id: log.defect_case_id || '',
            source: log.source || '',
    ...
    function applyConsumeLogSelection(m, id) {
        // 행 클릭/로드는 하이라이트(선택)만 — 체크는 ㅁ를 직접 눌러야 함
        if (!id) return;
    ...
    function syncConsumeLogHistHeadPad() {
        const scroll = document.getElementById('consumeLogHistListScroll');
        const head = document.getElementById('consumeLogHistListHead');
        if (!scroll || !head) return;
        const sb = scroll.offsetWidth - scroll.clientWidth;
        head.style.paddingRight = sb > 0 ? `${sb}px` : '';
    }

    /** Soft-refresh list rows/count/toolbar only — keep Period/Search inputs (focus & typing). */
    async function patchConsumeLogListUi() {
        const { st, vesselId } = await vesselScope();
        const m = modState(st);
        const allLogs = await TVC_Inventory.listConsumeLogs(vesselId);
        const logs = filterConsumeLogs(allLogs, st);
        const rows = buildConsumeLogRowsHtml(logs);
        const countLabel = `${logs.length}${logs.length !== allLogs.length ? ` / ${allLogs.length}` : ''} item(s)`;

        document.querySelectorAll('#consumeLogHistPanel .spare-req-list-count, #spareConsumeLogBody .spare-req-list-count')
            .forEach(el => { el.textContent = countLabel; });

        const histTbody = document.querySelector('#consumeLogHistListScroll .spare-consume-log-body-table tbody');
        const modalTbody = document.querySelector('#spareConsumeLogScroll .spare-consume-log-body-table tbody');
        if (histTbody) histTbody.innerHTML = rows;
        if (modalTbody) modalTbody.innerHTML = rows;

        updateConsumeLogHeadCheckAll(logs);

        const isHq = isReqListHqUser(st);
        const canConfirm = consumeLogAnyCheckedCanConfirm(m, allLogs, st);
        const canApprove = isHq && consumeLogAnyCheckedCanApprove(m, allLogs, st);
        const canDelete = consumeLogCanDeleteSelection(m, st);
        const toolbarRoots = [
            document.getElementById('consumeLogHistPanel'),
            document.getElementById('spareConsumeLogBody'),
        ].filter(Boolean);
        toolbarRoots.forEach(root => {
            setSpareListToolbarBtnDisabled(root, 'consumeLogReportConfirm', !canConfirm);
            if (isHq) setSpareListToolbarBtnDisabled(root, 'consumeLogReportApprove', !canApprove);
            setSpareListToolbarBtnDisabled(root, 'consumeLogDelete', !canDelete);
        });

        document.querySelectorAll('#consumeLogSearch').forEach(searchEl => {
            const clearBtn = searchEl.parentElement?.querySelector('.search-clear-btn');
            if (clearBtn) clearBtn.classList.toggle('hidden', !m.consumeLogSearch);
        });
        const periodEl = document.getElementById('consumeLogPeriodFilter');
        if (periodEl) periodEl.classList.toggle('active', !!(m.consumeLogPeriodFrom || m.consumeLogPeriodTo));
        if (typeof TVC_ListFilters !== 'undefined') TVC_ListFilters.syncBtn('consumeLog');
    }

    function consumeLogListCanSoftPatch() {
        if (_consumeLogHistOpen && document.querySelector('#consumeLogHistListScroll .spare-consume-log-body-table tbody')) {
            return true;
        }
        const modal = document.getElementById('spareConsumeLogModal');
        return !!(modal && !modal.classList.contains('hidden')
            && document.querySelector('#spareConsumeLogScroll .spare-consume-log-body-table tbody'));
    }

    async function refreshConsumeLogListUi(opts = {}) {
        if (!opts.full && consumeLogListCanSoftPatch()) {
            await patchConsumeLogListUi();
            return;
        }
        if (_consumeLogHistOpen) {
    ...
    async function syncConsumeLogHistPopoverToolbar() {
        ...
        const canApprove = isHq && consumeLogAnyCheckedCanApprove(m, allLogs, st);
        const canDelete = consumeLogCanDeleteSelection(m, st);
        setSpareListToolbarBtnDisabled(panel, 'consumeLogReportConfirm', !canConfirm);
        if (isHq) setSpareListToolbarBtnDisabled(panel, 'consumeLogReportApprove', !canApprove);
    ...
    function setConsumeLogHistOpen(open, opts = {}) {
        _consumeLogHistOpen = open;
        ...
        if (open) {
            requestAnimationFrame(() => {
                ...
                syncConsumeLogHistHeadPad();
            });
        }
    }

    async function refreshConsumeLogHistList() {
        const panel = document.getElementById('consumeLogHistPanel');
        if (!panel) return;
        const { st, vesselId } = await vesselScope();
        const m = modState(st);
        const listMode = isConsumedLogWindow(m);
        const allLogs = await TVC_Inventory.listConsumeLogs(vesselId);
        const logs = filterConsumeLogs(allLogs, st);
        const rows = buildConsumeLogRowsHtml(logs);
        const isHq = isReqListHqUser(st);
        ...
        let headHtml;
        if (listMode) {
            const approveBtn = isHq
                ? consumeLogToolbarBtn('Approve', 'TVC_SpareMenu.consumeLogReportApprove()', !canApprove, 'btn-green')
                : '';
            headHtml = `<div class="spare-req-list-head spare-req-hist-popover-head spare-consume-log-toolbar">
                <h3 class="spare-req-work-title">📋 Consumption List
                    <span class="muted spare-req-list-count">${countLabel}</span>
                </h3>
                <span class="spare-req-work-head-spacer"></span>
                ${showConfirm ? consumeLogToolbarBtn('Confirm', 'TVC_SpareMenu.consumeLogReportConfirm()', !canConfirm, 'btn-green') : ''}
                ${approveBtn}
                ${consumeLogToolbarBtn('Delete', 'TVC_SpareMenu.consumeLogDelete()', !canDelete, 'btn-red')}
                <span class="orig-toolbar-sep" aria-hidden="true"></span>
                ${consumeLogToolbarBtn('Print', 'TVC_SpareMenu.consumeLogPrint()')}
                ${consumeLogToolbarBtn('Preview', 'TVC_SpareMenu.consumeLogDocPreview()')}
                <button type="button" class="modal-x" onclick="TVC_SpareMenu.toggleConsumeLogHistList()" title="Close">×</button>
            </div>`;
        } else {
            headHtml = `<div class="spare-req-hist-popover-head">${consumeLogHistPickBtnLabel(false)}
                <span class="muted spare-req-list-count">${countLabel}</span>
                <button type="button" class="modal-x" onclick="TVC_SpareMenu.toggleConsumeLogHistList()" title="Close">×</button></div>`;
        }
        const hintText = listMode
            ? 'Click a row to load consumption log.'
            : 'Click a row to fill File No.';
        panel.innerHTML = `${headHtml}
            ${renderConsumeLogFiltersHtml(m)}
            <div class="spare-req-list-panel-wrap spare-req-hist-panel-wrap">
                <div class="panel spare-req-list-panel spare-consume-log-panel spare-req-hist-list-panel">
                  <div class="spare-req-list-head-wrap" id="consumeLogHistListHead">
                    <table class="spare-data-table spare-req-list-table spare-consume-log-table spare-req-list-head-table spare-consume-log-head-table">${CONSUME_LOG_COLGROUP}${consumeLogTableHeadHtml('consumeLogHistHeadChkAll')}</table>
                  </div>
                  <div class="spare-req-hist-list-scroll" id="consumeLogHistListScroll">
        ...
        bindConsumeLogHistKeyboard();
        TVC_App.bindSearchClearInput?.('consumeLogSearch');
        TVC_App.updateSearchClearBtn?.('consumeLogSearch');
        positionConsumeLogHistPopover();
        requestAnimationFrame(() => {
            positionConsumeLogHistPopover();
            syncConsumeLogHistHeadPad();
            consumeLogHistScrollSelectedIntoView();
        });
    }

    function consumeSetFileNoFromLog(log) {
        if (!log) return;
        const page1 = resolveConsumeLogPage1Meta(log);
        const draft = getConsumeSession();
        if (draft) {
            draft.file_no = page1.file_no || '';
            draft.voy_no = page1.voy_no || '';
            draft.place = page1.place || '';
        }
        const setVal = (id, val) => {
            const el = document.getElementById(id);
            if (el) el.value = val || '';
        };
        setVal('consumeFileNo', page1.file_no);
        setVal('consumeVoyNo', page1.voy_no);
        setVal('consumePlace', page1.place);
        captureConsumeMeta();
    }

    async function toggleConsumeLogHistList() {
        if (_consumeLogHistOpen) {
            setConsumeLogHistOpen(false);
            return;
        }
        setConsumeLogHistOpen(true, { reset: true });
        await refreshConsumeLogHistList();
    }
    ...
    function applyConsumeMetaFromSession(draft, st = getState()) {
        ...
        setVal('consumeFileNo', draft.file_no || '');
        setVal('consumeVoyNo', draft.voy_no || '');
    ...
    async function startConsumeLogListSession(opts = {}) {
        ...
        closeAllConsumeMetaPicks();
        if (opts.openSelectList) setConsumeLogHistOpen(true, { reset: true });
    ...
    /** Consumption List 창에서 로그 열기 (조회 또는 편집) */
    async function openConsumeLogInListWindow(logId, { editing = false } = {}) {
        const st = getState();
        if (window.TVC_App?.switchTab && st.currentTab !== 'spare') {
            TVC_App.switchTab('spare');
        }
        const m = modState(st);
        if (!isConsumedLogWindow(m) || !m.consumeOpen) {
            await startConsumeLogListSession({ openSelectList: false });
        }
        await loadConsumeLogIntoListWindow(logId);
        m.consumeLogListEditing = !!editing;
        m.consumeEditMode = false;
        m.consumePreview = false;
        await renderConsumeModal();
        return true;
    ...
    async function consumeLogCancelEdit() {
        const st = getState();
        const m = modState(st);
    ...
    function buildConsumeLogRowsHtml(logs) {
        if (!logs.length) {
            return `<tr><td colspan="9" class="spare-req-list-empty">
                ...
                <p class="spare-req-list-empty-sub muted">Click <strong>New</strong> to create a consumption record, or adjust Period / Search.</p>
            </td></tr>`;
        }
        const m = modState(getState());
        return logs.map(log => {
            ...
            return `<tr class="spare-consume-log-row${sel ? ' sr-req-sel' : ''}" data-consume-log-id="${lid}"
                ...
                ${consumeLogTypeCell(log)}
                <td class="spare-consume-log-file"${cellTitleAttr(fileNo)}>${esc(fileNo || '—')}</td>
                <td class="spare-consume-log-job"${cellTitleAttr(jobText)}>${jobText ? `<strong>${esc(jobText)}</strong>` : '—'}</td>
    ...
    function openConsumeLogModal(opts = {}) {
        ...
        renderConsumeLogModal().then(() => {
            showSpicsModal('spareConsumeLogModal');
            bindConsumeLogHistKeyboard();
        });
    }
    ...
    function viewConsumedLog() {
        openConsumedLogWindow();
    }

    async function openConsumedLogWindow() {
        clearConsumeLogUiState(modState(getState()));
        await startConsumeLogListSession({ openSelectList: true });
    }
    ...
    async function consumeLogSelectRow(id) {
        ...
        if (isConsumedLogWindow(m)) {
            await loadConsumeLogIntoListWindow(id, { preserveHistPopover: true });
        } else {
            await refreshConsumeLogListUi();
        }
        const scroll = document.getElementById('consumeLogHistListScroll')
            || document.getElementById('spareConsumeLogScroll');
        if (scroll && document.activeElement?.closest?.('.spare-consume-log-filters') == null
            && document.activeElement?.tagName !== 'INPUT'
            && document.activeElement?.tagName !== 'TEXTAREA') {
            scroll.setAttribute('tabindex', '-1');
            try { scroll.focus({ preventScroll: true }); } catch (_) { /* ignore */ }
    ...
    async function consumeLogReportConfirm() {
        ...
        const idsToConfirm = checkedIds.length
            ? consumeLogCheckedConfirmableIds(m, allLogs, st)
            : [];
        ...
        for (const id of idsToConfirm) {
            ...
            if (!log || !consumeLogCanConfirm(st, log)) continue;
            const user = spareInventoryUser(st);
        ...
        if (needReload && isConsumedLogWindow(m)) {
            await loadConsumeLogIntoListWindow(loadedId, { preserveHistPopover: true });
        }
        await refreshConsumeLogListUi();
    ...
    async function consumeLogReportApprove() {
        ...
        const idsToApprove = checkedIds.length
            ? consumeLogCheckedApprovableIds(m, allLogs, st)
            : [];
        ...
        for (const id of idsToApprove) {
            const log = await TVC_Inventory.getConsumeLog(id);
            if (!log || !consumeLogCanApprove(st, log)) continue;
            if (!log.confirmed_at && !log.confirmed_by && TVC_RBAC.canHqDirectApprove(user, log)) {
    ...
    async function consumeLogDocPreview() {
        ...
        openSpareListPrintWindow('Consumption List', body, true);
    }

    async function beginConsumeLogListNew() {
        const { st } = await vesselScope();
        const user = spareInventoryUser(st);
        ...
        const m = modState(st);
        if (m.consumeLogListEditing) return false;
        _consumeDraft = newConsumeDraft(st, user);
        syncConsumeLineMap();
        m.consumeLogListMode = true;
        m.consumeOpen = true;
    ...
    async function consumeLogModify() {
        ...
        await loadConsumeLogIntoListWindow(m.selectedConsumeLogId);
        m.consumeLogListEditing = true;
        await renderConsumeModal();
    }
    ...
    async function consumeLogDelete() {
        ...
        for (const id of idsToDelete) {
            const log = await TVC_Inventory.getConsumeLog(id);
            labels.push(log
                ? ([log.file_no, log.job_code, log.made_on].filter(Boolean).join(' · ') || id)
                : id);
        }
        ...
        for (const id of idsToDelete) {
            ...
            const isWrLinked = log.source === 'work_report' || !!log.work_report_id
                || log.source === 'defect_report' || !!log.defect_case_id;
            if (!isWrLinked && log.stock_applied_at && (log.lines || []).length && user) {
            ...
            await TVC_Inventory.deleteConsumeLog(id);
            if (loadedId === id) needClearLoaded = true;
        }
        m.selectedConsumeLogId = null;
        m.consumeLogCheckedIds = {};
        if (needClearLoaded && isConsumedLogWindow(m)) {
            _consumeDraft = newConsumeDraft(st, user);
            syncConsumeLineMap();
            m.consumeLogListEditing = false;
            m.consumeLastSavedLogId = null;
    ...
    async function consumeLogPreview() {
        ...
        closeConsumeLogModal();
        await openConsumeLogInListWindow(id, { editing: false });
    }

    async function consumeLogPrint() {
        const body = await buildConsumedLogListPrintBody();
        openSpareListPrintWindow('Consumption List', body, false);
    }
    ...
    function buildConsumeLogPreviewPagesHtml(st, log, vesselName) {
        ...
        return `<div class="req-preview-page">
            ...
            <table class="req-preview-meta">
                <tbody>
                    <tr><th>Vessel Name</th><td>${esc(vesselName)}</td><th>Work Date</th><td>${esc(log.consumed_date || '—')}</td></tr>
    ...
    function renderConsumePreviewHtml(st, draft, vesselName) {
        if (!draft) return '';
        const pages = buildConsumeLogWrPrintHtmlFromDraft(st, draft, { innerOnly: true });
        const tone = consumeLogPrintTone({
            source: draft.source,
            work_report_id: draft.work_report_id,
            defect_case_id: draft.defect_case_id,
        });
        return `
        <div class="spare-req-work-wrap req-preview-wrap wr-consume-preview-wrap">
          ...
          <div class="spare-req-work-scroll req-preview-scroll wr-consume-preview-scroll">
            ...
            <div class="wr-consume-preview-doc df-print-root"><div class="wr-page tone-${escAttr(tone)}">${pages}</div></div>
          </div>
        </div>`;
    }

    async function ensureConsumeLinkedEntities(log, st) {
        if (!log) return;
        const type = consumeLogTypeMarker(log);
        if (type.letter === 'M') {
            let rep = consumeLogLinkedReport(log, st);
            if (!rep && log.work_report_id && window.TVC_DB) {
                try { rep = await TVC_DB.get('daily_work_reports', log.work_report_id); } catch (_) { /* ignore */ }
            }
            if (!rep && log.id && window.TVC_DB) {
                try {
                    const all = await TVC_DB.getAll('daily_work_reports');
            ...
            if (rep) {
                st.reports = st.reports || [];
                if (!st.reports.some(r => r.id === rep.id)) st.reports.push(rep);
            }
        } else if (type.letter === 'D') {
            let dc = consumeLogLinkedDefect(log, st);
            if (!dc && log.defect_case_id && window.TVC_DB) {
                try { dc = await TVC_DB.get('defect_cases', log.defect_case_id); } catch (_) { /* ignore */ }
            }
            if (!dc && log.id && window.TVC_DB) {
                try {
                    const all = await TVC_DB.getAll('defect_cases');
                    dc = (all || []).find(d => d.consume_log_id === log.id) || null;
                } catch (_) { /* ignore */ }
            }
            if (dc) {
                st.defectCases = st.defectCases || [];
                if (!st.defectCases.some(d => d.id === dc.id)) st.defectCases.push(dc);
            }
        }
    }

    function consumeLogPrintTitle(st, log) {
        const type = consumeLogTypeMarker(log);
        if (type.letter === 'M') return 'Maintenance Report';
        if (type.letter === 'D') {
            const dc = consumeLogLinkedDefect(log, st);
            return dc ? `Defect Report ${dc.case_no || ''}`.trim() : 'Defect Report';
        }
        if (type.letter === 'P') return 'Postponed Report';
        return 'Consumption Report';
    }

    function consumeLogPrintTone(log) {
        const type = consumeLogTypeMarker(log);
        if (type.letter === 'D') return 'defect';
        if (type.letter === 'P') return 'postpone';
        return 'repair';
    }
    ...
    async function buildConsumeLogPrintDocument(logId) {
        ...
        const log = await TVC_Inventory.getConsumeLog(logId);
        if (!log) return null;
        await ensureConsumeLinkedEntities(log, st);
        return {
            title: consumeLogPrintTitle(st, log),
            html: buildConsumeLogWrPrintHtml(st, log),
        };
    }
    ...
    async function consumeLogPrintPreview() {
        ...
        const doc = await buildConsumeLogPrintDocument(id);
        if (!doc?.html) {
            await TVC_Dialog.alert('Consumption log not found.');
            return;
        }
        openWrReportPrintWindow(doc.title, doc.html, { print: false, appCss: true });
    }
    ...
    async function consumeLogOpenList() {
        const m = modState(getState());
        if (isConsumedLogWindow(m)) {
            await toggleConsumeLogHistList();
            return;
        }
        const selectId = m.consumeLastSavedLogId || null;
        closeConsumeModal({ skipConfirm: true });
    ...
    async function reqListPreview() {
        ...
        closeReqListModal();
        await openRequisitionInListWindow(id, { editing: false });
    }
    ...
    async function reqListReportApprove() {
        ...
        for (const id of idsToApprove) {
            ...
            if (!req.confirmed_at && !req.confirmed_by && TVC_RBAC.canHqDirectApprove(user, req)) {
                req.confirmed_by = requisitionConfirmByLabel(st, req);
    ...
    function startInlineAppend() {
        ...
        const header = resolveAppendGroupHeader(st);
        m.inlineDraft = {
            header,
    ...
    function pickEditGroup(label) {
        ...
        if (m.inlineDraft?.header) {
            ...
            m.inlineDraft.header.equipment = '';
    ...
    function onEditEquipmentInput() {
        ...
        let header;
        if (equipment) {
            const equipDef = groupDefHeader(st, groupLabel, equipment);
            header = {
                pmsGroupNo: groupLabel,
                equipment,
                machineryName: equipDef?.machineryName || equipment,
        ...
        if (m.inlineDraft?.header) {
            m.inlineDraft.header.equipment = equipment;
        }
    ...
    async function saveGroupHeaderEdit() {
        ...
        const equipment = g('sgh_g_equipment');
        const header = {
            pmsGroupNo: newLabel,
            equipment,
            // Machinery Name / Ass'y Name / Dwg. No.는 헤더 UI에서 제거됨 → 기존 값 보존
            machineryName: draft.machineryName || equipment || '',
        ...
        try {
            if (equipment) {
                await saveJobEquipmentHeader(spareInventoryUser(st), {
                    ...
                    equipment_no: equipmentNoForName(st, newLabel, equipment),
                    maker: header.maker,
                ...
                afterSpareListChange(st);
                if (window.TVC_App?.syncSpareItemToolbar) TVC_App.syncSpareItemToolbar();
                await TVC_Dialog.alert(`Equipment header saved for “${equipment}”.`);
                return;
            }
    ...
    async function saveGroupHeaderMeta(st, header, spare, opts = {}) {
        ...
        const secCode = sectionCodeFromSpare(spare);
    ...
    async function saveInlineEdit() {
        ...
        const equipment = g('sgh_equipment').trim();
        const header = {
        ...
        const groupNo = typeof TVC_SpareCode !== 'undefined'
            ? TVC_SpareCode.groupNoFromLabel(header.pmsGroupNo)
            : spareCodeGroupNo({ inventoryNumbering: code });
        const equipNo = equipmentNoForName(st, header.pmsGroupNo, equipment);
        let finalCode = code;
        if (typeof TVC_SpareCode !== 'undefined' && groupNo) {
            finalCode = TVC_SpareCode.normalizeCode(finalCode, groupNo, equipNo);
            if (!TVC_SpareCode.isStandard(finalCode)) {
                finalCode = TVC_SpareCode.nextInBlock(
                    (st.spares || []).map(canon).filter(s => s.id !== id),
                    groupNo,
                    equipNo,
                );
            }
        ...
        const updated = {
            ...baseCanon,
            makerPartNo: partNo || finalCode,
            inventoryNumbering: finalCode,
            name: itemName,
            partClass,
            isCritical: partClass === 'L',
            dwgNo: g('sie_dwg').trim(),
        ...
        try {
            ...
            if (equipment) {
                await saveJobEquipmentHeader(spareInventoryUser(st), {
                    department: st.department || draft.category || '',
                    group: header.pmsGroupNo,
                    item_sort1: equipment,
    ...
    function spareHistCategoryKey(row, { hq = false } = {}) {
        const c = String(row?.category || '').toUpperCase();
        if (c === 'RECEIVED') return 'received';
        if (c === 'INVENTORY') return 'inventory';
        if (c === 'REPLY_EVALUATION' || c === 'ASSESSMENT') return 'evaluation';
        if (hq) {
            if (c === 'QUOTATION') return 'quotation';
            if (c === 'PURCHASE_ORDER') return 'order';
        }
        // Vessel: Quotation / Order 이력은 Requisition 탭에 포함
        return 'requisition';
    }
    ...
    function spareHistTabsForUser(user) {
        if (window.TVC_RBAC?.isHqAccount?.(user)) {
            return ['requisition', 'quotation', 'evaluation', 'order', 'received', 'inventory'];
        }
        return ['requisition', 'evaluation', 'received', 'inventory'];
    }

    function spareHistNormalizeCategory(key, user) {
        const tabs = spareHistTabsForUser(user);
        return tabs.includes(key) ? key : tabs[0];
    }
    ...
    function renderSpareXferModal() {
        ...
        if (step === 'mode') {
            ...
            if (isHq) {
                ...
                const typeBtns = VESSEL_IMPORT_TYPES.map(t => `
                    <button type="button" class="btn spare-sync-btn spare-sync-check-btn${vesselType === t.key ? ' is-checked' : ''}"
                        aria-pressed="${vesselType === t.key ? 'true' : 'false'}"
                ...
                <div class="spare-sync-actions">
                    ${typeBtns}
                    <button type="button" class="btn btn-green spare-sync-btn"
                        onclick="TVC_SpareMenu.spareXferTriggerImport('vessel')"
                        ${vesselType ? '' : ' disabled title="Select import type first"'}>Open file…</button>
                </div>`;
            }
        }
    ...
    function spareXferSelectVesselImportType(key) {
        const k = String(key || '');
    ...
    /** 작성자(Engineer / Officer) — Data Export·History·Inventory Update 숨김 */
    ...
    function renderSpicsMenuHtml({ canConsume, canDeliver, canRequisition, canHqImport, canModify, user, isHq }) {
        ...
        const consumedItems = [
            item('Consumption List', 'TVC_SpareMenu.viewConsumedLog()', canConsume, true),
        ].join('');
        ...
        <nav class="spare-flow-panel" aria-label="SPARE workflow">
          ...
          ${col('req', 'Requisition', [
                item('Requisition List', 'TVC_SpareMenu.viewRequisitionList()', canRequisition, true),
            ].join(''))}
    ...
    /* ── Requisition List window (reqWorkListMode) — unified list + in-window New/Modify ── */
    ...
    function finalizeReqWorkDraftForSave() {
        ...
        } else if (!req.reporter_role && user) {
            req.reporter_role = TVC_RBAC.resolveUserRole(user) || user.role || '';
        }
        if (req.req_no?.trim()) {
    ...
    function consumeLogApprovalState(record, department, opts = {}) {
        ...
        const listEditing = !!opts.listEditing;
        const isHq = isReqListHqUser(st);
        const canConfirmNow = hasKey && consumeLogCanConfirm(st, { ...record, department: dept })
            && (!listMode || listEditing);
        const canApproveNow = hasKey && consumeLogCanApprove(st, { ...record, department: dept })
            && (!listMode || listEditing || isHq);
        return {
            isConfirmed,
            isApproved,
    ...
    function spareConfirmedByToggle(prefix) {
        ...
        const record = prefix === 'reqWork' ? _reqWorkDraft
            : (prefix === 'consumeLog' ? _consumeDraft : null);
        if (cb.checked) {
            if (prefix === 'closeOs') {
    ...
    function applySpareApprovalFromUi(record, prefix, department) {
        ...
        if (apCb?.checked && !apCb.disabled && canApprove) {
            if (!record.approved_by && !record.approved_at) {
                // HQ 작성분 직접 Approve 시 Confirmed도 함께 기록
                if (!record.confirmed_at && !record.confirmed_by && TVC_RBAC.canHqDirectApprove(user, record)) {
                    record.confirmed_by = prefix === 'reqWork'
                        ? requisitionConfirmByLabel(st, record)
                        : (TVC_RBAC.getDepartmentConfirmLabel(dept) || TVC_RBAC.getRankLabel(user));
                    record.confirmed_at = now;
    ...
    async function tryApplySpareApprovalOnClose(kind, record, department) {
        ...
        const prefix = kind === 'consume'
            ? (isConsumedLogWindow() ? 'consumeLog' : 'consume')
            : 'reqWork';
        if (!applySpareApprovalFromUi(record, prefix, department)) return false;
    ...
    function renderReqWorkMetaHtml(req, opts = {}) {
        ...
        const isHq = isReqListHqUser(st);
        const listMode = !!opts.listMode;
        const listEditing = !!modState(st).reqWorkListEditing;
        const evalEditable = isHq && !preview && !docPreview;
        ...
        return `<section class="spare-req-work-meta" aria-label="Requisition information">
            ...
            <div class="spare-req-meta-grid">
                ...
                <div class="spare-req-meta-comments-row">
                    <div class="spare-consume-meta-comments">
                        <label class="spare-consume-meta-col-head" for="reqWorkShipComments">Ship's Comments (If any)</label>
                        <textarea id="reqWorkShipComments" class="spare-req-meta-input spare-consume-meta-textarea spare-req-meta-comments-textarea" rows="4"
                            placeholder="Enter ship-side remarks for this requisition…"${commentsRo}${commentsOnInput}>${esc(req.ships_comments || '')}</textarea>
                    </div>
                    <div class="spare-consume-meta-comments">
    ...
    async function patchReqWorkHistListUi() {
        const panel = document.getElementById('reqWorkHistPanel');
        if (!panel) return;
        const st = getState();
        const { vesselId } = await vesselScope();
        ...
        const reqRows = buildReqListRowsHtml(reqs, listMode ? 'modal' : 'pick');
        const countLabel = `${reqs.length}${reqs.length !== allReqs.length ? ` / ${allReqs.length}` : ''} item(s)`;

        panel.querySelectorAll('.spare-req-list-count').forEach(el => { el.textContent = countLabel; });
        const tbody = panel.querySelector('#reqWorkHistListScroll .spare-req-list-body-table tbody');
        if (tbody) tbody.innerHTML = reqRows;
        updateReqListHeadCheckAll(reqs);
    ...
    async function refreshReqWorkHistList(opts = {}) {
        const panel = document.getElementById('reqWorkHistPanel');
        if (!panel) return;
        if (!opts.full && panel.querySelector('#reqHistPeriodFrom')
            && panel.querySelector('#reqWorkHistListScroll .spare-req-list-body-table tbody')) {
            await patchReqWorkHistListUi();
            return;
        }
        const st = getState();
        const { vesselId } = await vesselScope();
        ...
        panel.innerHTML = `${headHtml}
            ${renderReqListFiltersHtml(m, { prefix: 'reqHist' })}
            <div class="spare-req-list-panel-wrap spare-req-hist-panel-wrap">
    ...
    async function toggleReqWorkHistList() {
        ...
        await refreshReqWorkHistList({ full: true });
    }
    ...
    async function renderReqWorkModal() {
        ...
        const reqWindowTitle = sparePartsRequisitionWindowTitle(m);
        const completeDone = !!m.reqWorkCompleted;
        const preview = false;
        ...
        const countLabel = isRequisitionListWindow(m)
            ...
            ? reqWorkSelectedCountLabel(st, _reqWorkCachedList.length)
            : `${_reqWorkCachedList.length} / ${allCanon.length}`;
        const modifyHint = (isRequisitionListWindow(m) && m.reqWorkListEditing) && !m.reqWorkCompleted
            ? '<p class="spare-req-work-modify-hint muted">Use checkboxes to add/remove parts · Enter Request qty, then Save</p>'
            : '';
        const headTitle = preview ? 'Preview' : reqWindowTitle;
        ...
        const headModifyBtn = listMode && !docPreview
            ...
            : '';
        const headCloseOsBtn = listMode && !docPreview && !isSpareAuthorAccount(spareInventoryUser(st))
        ...
        const searchBar = (preview || docPreview) ? '' : `<div class="filter-bar spare-list-search-bar">
                ...
                <button type="button" class="search-clear-btn hidden" title="Clear search" aria-label="Clear search"
                    onclick="TVC_App.clearSearchField('reqWorkSearch')">×</button>
                </div>
        ...
        if (!preview && !docPreview) {
            ...
            updateSpareModalTreeToggleUi('reqWork');
            syncSpareItemHistoryBtns();
            TVC_App.bindSearchClearInput?.('reqWorkSearch');
            TVC_App.updateSearchClearBtn?.('reqWorkSearch');
        }
        if (preview || docPreview) {
    ...
    async function reqWorkSave() {
        ...
        if (isRequisitionListWindow(m)) {
            ...
            if (pendingConfirm && spareListStatus(_reqWorkDraft) === SPARE_LIST_STATUS.CONFIRMED) {
                ...
                await TVC_Dialog.alert(`Requisition ${_reqWorkDraft.req_no} saved (${_reqWorkDraft.lines.length} line(s)).`);
            }
            return;
        }
    }
    ...
    const CONSUME_NO_GROUP_KEY = '__NO_GROUP__';
    const CONSUME_NO_GROUP_LABEL = 'No PMS GROUP';
    ...
    function renderConsumeMetaFields() {
        ...
        set('consumeFileNo', draft.file_no || '');
        set('consumeVoyNo', draft.voy_no || '');
        set('consumePlace', draft.place || '');
        set('consumeShipComments', draft.ships_comments);
        const groupText = document.getElementById('consumeGroupPickText');
        if (groupText) {
            groupText.textContent = consumeGroupDisplayLabel(draft) || '— Select PMS Group —';
        }
        renderConsumeJobRowsSection();
    }

    function renderConsumeMaintJobRowHtml(row, idx, opts = {}) {
        ...
        if (ro) {
            ...
            jobInner = `<button type="button" id="consumeJobPickTrigger-${idx}" class="wr-maint-job-pick spare-consume-job-pick-trigger"${jobDisabled ? ' disabled' : ''} onclick="TVC_SpareMenu.toggleConsumeJobPick(event, ${idx})">
                    <span class="spare-consume-pick-text">${esc(row.job_code || '— No Job Code —')}</span>
        ...
        const sort1Inner = roInp(row.sort1, 'sort1');
        const sort2Inner = roInp(row.sort2, 'sort2');
        const detailInner = ro
            ? roInp(row.job_detail, 'job_detail')
            : `<input type="text" id="consumeJobDetail-${idx}" data-field="job_detail" value="${esc(row.job_detail || '')}" onchange="TVC_SpareMenu.captureConsumeMeta()">`;
        const actCol = batch && !ro && (opts.rowCount || 0) > 1
            ? `<div class="wr-maint-field df-maint-job-row-act${hideLabels ? ' wr-maint-field-nolabel' : ''}">${hideLabels ? '' : '<label aria-hidden="true">&nbsp;</label>'}<button type="button" class="btn btn-sm spare-consume-job-row-rm" onclick="TVC_SpareMenu.removeConsumeJobRow(${idx})" title="Remove job row" aria-label="Remove job row">×</button></div>`
            : '';
        const gapCls = idx === 0 ? ' wr-maint-grid-gap' : '';
        const gridCols = actCol ? ' wr-maint-grid-4 df-maint-job-grid-batch' : ' wr-maint-grid-4';
        return `<div class="wr-maint-grid${gridCols}${gapCls} df-maint-job-row wr-page2-job-row" data-consume-job-row="${idx}">
    ...
    function renderConsumeJobRowFieldHtml(row, idx, opts = {}) {
        if (opts.maintStyle) return renderConsumeMaintJobRowHtml(row, idx, opts);
        const ro = !!opts.readonly;
        const batch = !!opts.batch;
    ...
    function renderConsumeJobRowsInner(draft, opts = {}) {
        ...
        if (opts.maintStyle) {
            const allowAdd = batch;
            const multiJob = items.length > 1 || allowAdd;
            const maintMulti = multiJob;
            const header = maintMulti
                ? renderMaintJobRowsHeaderHtml({ withActionCol: allowAdd && items.length > 1 })
                : '';
            const rows = items.map((row, idx) => renderConsumeJobRowFieldHtml(row, idx, {
                ...
                maintStyle: true,
                hideLabels: maintMulti,
            })).join('');
            const addBtn = allowAdd && !opts.readonly && draft.spare_group_key
    ...
    function renderConsumeJobRowsSection() {
        ...
        container.innerHTML = renderConsumeJobRowsInner(draft, { readonly: ro, maintStyle });
    ...
    function renderMaintJobRowsHeaderHtml(opts = {}) {
        const withActionCol = !!opts.withActionCol;
        const act = withActionCol
            ? '<div class="wr-maint-job-col-head wr-maint-job-col-head-act" aria-hidden="true">&nbsp;</div>'
            : '';
        const batchCls = withActionCol ? ' df-maint-job-grid-batch' : '';
        return `<div class="wr-maint-grid wr-maint-grid-4${batchCls} wr-maint-job-rows-header wr-maint-grid-gap">
            <div class="wr-maint-job-col-head">Job Code</div>
            <div class="wr-maint-job-col-head">SORT-1</div>
            <div class="wr-maint-job-col-head">SORT-2</div>
            <div class="wr-maint-job-col-head">Job Detail</div>
            ${act}
        </div>`;
    }

    function renderPage2MaintJobHeaderHtml(opts = {}) {
        return renderMaintJobRowsHeaderHtml(opts);
    }
    ...
    function renderPage2MaintJobRowHtml(row, idx, opts = {}) {
        ...
        const actCol = batch && !ro && (opts.rowCount || 0) > 1
            ? `<div class="wr-maint-field wr-page2-job-row-act${hideLabels ? ' wr-maint-field-nolabel' : ''}">${hideLabels ? '' : '<label aria-hidden="true">&nbsp;</label>'}<button type="button" class="btn btn-sm spare-consume-job-row-rm" onclick="TVC_SpareMenu.removePage2JobRow(${idx})" title="Remove job row" aria-label="Remove job row">×</button></div>`
            : '';
        const gapCls = idx === 0 ? ' wr-maint-grid-gap' : '';
    ...
    function renderPage2JobRowsInner(ctx, opts = {}) {
        ...
        const multiJob = items.length > 1 || allowAdd;
        const maintMulti = !!opts.maintStyle && multiJob;
        const header = maintMulti
            ? renderPage2MaintJobHeaderHtml({ withActionCol: allowAdd && items.length > 1 })
            : '';
    ...
    function consumeGroupDisplayLabel(draft) {
        if (!draft) return '';
        if (draft.spare_group_key === CONSUME_NO_GROUP_KEY || draft.spare_group_label === CONSUME_NO_GROUP_LABEL) {
            return CONSUME_NO_GROUP_LABEL;
        }
        return draft.spare_group_label ? safeTreeLabel(draft.spare_group_label) : '';
    }
    ...
    function buildConsumeGroupPickListInner(st) {
        const draft = getConsumeSession();
        const selKey = draft?.spare_group_key || '';
        ...
        let html = '';
        if (matchNoSelection) {
            const sel = selKey === CONSUME_NO_GROUP_KEY ? ' selected' : '';
            html += `<button type="button" class="spare-consume-pick-item spare-consume-pick-item-none${sel}"
                onclick="TVC_SpareMenu.pickConsumeMetaGroup('${escAttr(CONSUME_NO_GROUP_KEY)}','${escAttr(CONSUME_NO_GROUP_LABEL)}')">${esc(CONSUME_NO_GROUP_LABEL)}</button>`;
        }
        const nodes = planGroupNodes(st).filter(matchNode);
        if (!nodes.length && !matchNoSelection) {
            return q
                ? '<div class="spare-consume-pick-empty muted">No search results</div>'
                : '<div class="spare-consume-pick-empty muted">Loading PMS GROUP Tree…</div>';
        }
        let curDept = '';
        nodes.forEach(n => {
            if (n.department !== curDept) {
    ...
    function buildConsumeJobPickListInner(st) {
        ...
        const clearBtn = `<button type="button" class="spare-consume-pick-item spare-consume-pick-item-none${selectedCode ? '' : ' selected'}"
                onclick="TVC_SpareMenu.clearConsumeJobRow()">
    ...
    function clearConsumeJobRow() {
        captureConsumeMeta();
        const draft = getConsumeSession();
        if (!draft) return;
        const idx = _consumeActiveJobRowIndex || 0;
        ensureConsumeJobItems(draft);
        draft.job_items[idx] = newConsumeJobRow();
        syncConsumeDraftJobSummary(draft);
        closeConsumeJobPickMenu();
        renderConsumeJobRowsSection();
    }
    ...
    function renderConsumeMetaHtml(draft, opts = {}) {
        ...
        const st = getState();
        const listMode = !!opts.listMode;
        const listEditing = !!modState(st).consumeLogListEditing;
        const ro = !!opts.readonly || (listMode && !listEditing);
        const roAttr = ro ? ' readonly disabled tabindex="-1"' : '';
        const reportedByVal = resolveSpareReportedBy(draft, spareInventoryUser(st));
        const today = new Date().toISOString().slice(0, 10);
        const groupLabel = consumeGroupDisplayLabel(draft);
        ...
        const dateInp = (id, val, extra = '') =>
            `<input type="text" id="${id}" class="tvc-date-input${ro ? ' wr-ro' : ''}" placeholder="YYYY-MM-DD" autocomplete="off" value="${esc(fmtSpareDate(val))}"${roAttr}${extra}>`;
        const txtInp = (id, val) => ro
            ? `<input class="wr-ro" value="${esc(val || '')}" readonly tabindex="-1">`
            : `<input type="text" id="${id}" value="${esc(val || '')}" onchange="TVC_SpareMenu.captureConsumeMeta()">`;
        const showHistPick = !opts.readonly;
        const fileNoInner = showHistPick
            ? `<div class="spare-req-no-wrap">
                    <input type="text" id="consumeFileNo" class="spare-req-meta-input" value="${esc(draft.file_no || '')}" placeholder="File No." oninput="TVC_SpareMenu.captureConsumeMeta()" onchange="TVC_SpareMenu.captureConsumeMeta()">
                    <span class="spare-req-hist-anchor">
                        <button type="button" id="consumeLogHistBtn" class="btn btn-sm spare-req-hist-btn" onclick="TVC_SpareMenu.toggleConsumeLogHistList()">${consumeLogHistPickBtnLabel(listMode)}</button>
                    </span>
               </div>`
            : txtInp('consumeFileNo', draft.file_no || '');
        let pmsInner;
        if (ro) {
            pmsInner = `<input class="wr-ro" value="${esc(groupLabel || '—')}" readonly tabindex="-1">`;
        } else {
            pmsInner = `<div class="spare-consume-meta-pick" id="consumeGroupPick">
                    <button type="button" id="consumeGroupPickTrigger" class="wr-maint-job-pick spare-consume-pick-trigger" onclick="TVC_SpareMenu.toggleConsumeGroupPick(event)">
        ...
        const commentsOnInput = ro ? '' : ' oninput="TVC_SpareMenu.captureConsumeMeta()"';
        const approvalSection = listMode
            ? renderConsumeLogApprovalRow(draft, {
                prefix: 'consumeLog',
                department: opts.department || st.department,
                listMode: true,
                listEditing,
            })
            : '';
    ...
    function consumeListEditable() {
        const m = modState(getState());
        if (m.consumePreview) return false;
        if (isConsumedLogWindow(m)) return !!m.consumeLogListEditing;
        return !!m.consumeOpen;
    }
    ...
    function renderWrSpareMetaHtml(meta = {}, opts = {}) {
        ...
        return `<section class="wr-maint-card wr-maint-body wr-spare-meta-form" aria-label="Work report job context">
            ...
            ${(() => {
                ...
                const commentsDis = ro ? ' readonly disabled tabindex="-1"' : '';
                const commentsCls = ro ? ' wr-ro' : '';
                const commentsInput = ro ? '' : ' oninput="TVC_App.captureWorkReportForm()"';
                return fld("SHIP'S COMMENTS (IF ANY)", `<textarea id="wrSpareShipComments" class="wr-maint-textarea${commentsCls}" data-wf="spareShipComments" rows="3"${commentsDis}${commentsInput}>${esc(spareComments)}</textarea>`, 'wr-maint-span-all wr-maint-grid-gap');
            })()}
        </section>`;
    }
    ...
    <div class="filter-bar spare-list-search-bar">
      ...
      <button type="button" class="search-clear-btn hidden" title="Clear search" aria-label="Clear search"
        onclick="TVC_App.clearSearchField('wrSpareSearch')"${roDis ? ' disabled' : ''}>×</button>
      </div>
...
<div class="${layoutCls}">
  <main class="panel spare-main">
    ...
    function wrSpareNavigableSpares(st) {
        return (_wrSpareCachedList || []).filter(s => wrSpareRowChecked(s));
    }

    function findWrSpareRowTable(spareId) {
    ...
    function scrollWrSpareRowIntoView(spareId) {
        const scroll = document.getElementById('wrSpareListScroll');
        if (!scroll || !spareId) return;
        if (vlWrSpare && scroll.querySelector('.vl-inner')) {
            const idx = (_wrSpareCachedList || []).findIndex(s => wrSameSpareId(s.id, spareId));
            if (idx < 0) return;
            const rowH = TVC_VirtualList.ROW_H || 36;
            const viewH = scroll.clientHeight || 400;
            const rowTop = idx * rowH;
        ...
        findWrSpareRowTable(spareId)?.scrollIntoView({ block: 'nearest' });
    }

    function focusWrSpareQtyInput(spareId) {
        if (!spareId || !wrSpareListEditable()) return;
        const table = findWrSpareRowTable(spareId);
        const input = table?.querySelector('.spare-consume-qty-input:not([disabled])');
        if (input) {
            input.focus();
            input.select();
    ...
    function wrSpareApplyRowFocus(spareId, focusQty = false) {
        const st = getState();
        setFocusedSpareId(st, spareId || null);
        syncWrSpareGroupHeaderDom(st);
        scrollWrSpareRowIntoView(spareId);
        if (vlWrSpare) {
            vlWrSpare.refresh();
            updateWrSpareHeadCheckAll();
            requestAnimationFrame(() => {
        ...
        syncSpareItemHistoryBtns();
        if (focusQty && spareId && wrSpareListEditable()) {
            const spare = (st.spares || []).find(s => wrSameSpareId(s.id, spareId));
            if (spare && wrSpareRowChecked(spare)) {
                requestAnimationFrame(() => {
                    focusWrSpareQtyInput(spareId);
                    requestAnimationFrame(() => focusWrSpareQtyInput(spareId));
                });
            }
        }
    }

    function wrSpareMoveFocusRow(delta) {
        if (!wrSpareListEditable()) return;
        const st = getState();
        const list = wrSpareNavigableSpares(st);
        if (!list.length) return;
        const curId = getFocusedSpareId(st);
        let idx = list.findIndex(s => wrSameSpareId(s.id, curId));
        if (idx < 0) idx = delta > 0 ? -1 : list.length;
        const next = list[idx + delta];
        if (!next) return;
        wrSpareApplyRowFocus(next.id, true);
    }

    function onWrSpareListKeydown(e) {
        if (e.key !== 'ArrowUp' && e.key !== 'ArrowDown') return;
        if (!wrSpareListEditable()) return;
        const t = e.target;
        if (!t.closest?.('#wrSpareListScroll') && !t.classList?.contains('spare-consume-qty-input')) return;
        if (t.tagName === 'INPUT' && !t.classList.contains('spare-consume-qty-input')) return;
        if (t.tagName === 'TEXTAREA' || t.tagName === 'SELECT') return;
        e.preventDefault();
        wrSpareMoveFocusRow(e.key === 'ArrowDown' ? 1 : -1);
    }

    function bindWrSpareListKeyboard() {
        if (_wrSpareKeyBound) return;
        _wrSpareKeyBound = true;
        document.addEventListener('keydown', onWrSpareListKeydown, true);
    }

    function unbindWrSpareListKeyboard() {
        if (!_wrSpareKeyBound) return;
        _wrSpareKeyBound = false;
        document.removeEventListener('keydown', onWrSpareListKeydown, true);
    }

    function wrSpareFocusRow(spareId, focusQty = false) {
        if (spareId) wrSpareApplyRowFocus(spareId, focusQty);
        else {
            setFocusedSpareId(getState(), null);
            syncWrSpareGroupHeaderDom(getState());
            refreshWrSpareListRows();
            syncSpareItemHistoryBtns();
        }
    }
    ...
    function wrSpareToggleRow(spareId, checked) {
        ...
        if (checked) {
            if (!st._wrUsedParts.some(l => wrSameSpareId(l.spare_part_id, sid)) && spare) {
                st._wrUsedParts.push(buildWrLine(spare, 0));
            }
        } else {
            st._wrUsedParts = st._wrUsedParts.filter(l => !wrSameSpareId(l.spare_part_id, sid));
        }
        syncWrLineMap();
        if (modState(st).wrSpareShowSelectedOnly && !wrSpareCheckedCount(st)) {
            modState(st).wrSpareShowSelectedOnly = false;
        }
        if (checked && spare) wrSpareApplyRowFocus(spareId, false);
        else refreshWrSpareListUi();
    }
        ...
        if (checked) {
            list.forEach(s => {
                if (!st._wrUsedParts.some(l => wrSameSpareId(l.spare_part_id, s.id))) {
                    st._wrUsedParts.push(buildWrLine(s, 0));
                }
            });
        } else {
    ...
    function wrSpareSetQty(spareId, rawQty) {
        ...
        const spare = (st.spares || []).find(s => wrSameSpareId(s.id, sid));
        if (!line && spare) {
            line = buildWrLine(spare, qty);
            st._wrUsedParts.push(line);
        } else if (line) {
            line.qty_used = qty;
        }
        syncWrLineMap();
    ...
    function initWrSparePage2(ro, opts = {}) {
        ...
        updateWrSpareHeadStats();
        syncSpareItemHistoryBtns();
        if (!ro) bindWrSpareListKeyboard();
        hydrateWrSpareShipCommentsFromConsumeLog(st);
        TVC_App.bindSearchClearInput?.('wrSpareSearch');
        TVC_App.updateSearchClearBtn?.('wrSpareSearch');
        requestAnimationFrame(() => {
            syncWrSpareHeadLayout();
            requestAnimationFrame(syncWrSpareHeadLayout);
        });
    ...
    async function hydrateWrSpareShipCommentsFromConsumeLog(st) {
        if (!st?._wrReportId) return;
        st._wrForm = st._wrForm || {};
        if (String(st._wrForm.spareShipComments ?? '').trim()) return;
        const rep = (st.reports || []).find(r => r.id === st._wrReportId);
        if (!rep?.consume_log_id) return;
        try {
            const log = await TVC_Inventory.getConsumeLog(rep.consume_log_id);
            if (log?.ships_comments == null) return;
            st._wrForm.spareShipComments = log.ships_comments;
            const ta = document.getElementById('wrSpareShipComments');
            if (ta) ta.value = log.ships_comments;
        } catch (_) { /* ignore */ }
    }
    ...
    function persistWrSpareUsedParts() {
        ...
        if (scroll) {
            scroll.querySelectorAll('[data-spare-id]').forEach(el => {
                ...
                const qtyInp = el.querySelector('.spare-consume-qty-input:not([disabled])');
                if (cb && !cb.checked) {
                    st._wrUsedParts = st._wrUsedParts.filter(l => !wrSameSpareId(l.spare_part_id, sid));
                    return;
                }
    ...
    function renderWrPrintShell(title, activePage, bodyHtml, tone = 'defect', shellOpts = {}) {
        const hidePageTabs = !!shellOpts.hidePageTabs;
        return `<div class="wr-print-page">
            <div class="df-print-root">
                <div class="df-print-shell df-modal-inner">
                    ...
                    ${hidePageTabs ? '' : renderWrPrintPageTabs(activePage)}
    ...
    function wrReportPrintStyles() {
        return `
            ...
            * { box-sizing: border-box; }
            ...
            .wr-spare-print-table-wrap .spare-data-table-wrspare th,
            .wr-spare-print-table-wrap .spare-data-table-wrspare td,
            .wr-print-spare-table.spare-data-table-wrspare th,
            .wr-print-spare-table.spare-data-table-wrspare td,
            .wr-spare-print-layout .spare-data-table-wrspare th,
            .wr-spare-print-layout .spare-data-table-wrspare td {
                text-align: center !important;
            }
            .wr-spare-print-table-wrap .spare-data-table-wrspare .c-item,
            .wr-print-spare-table.spare-data-table-wrspare .c-item,
            .wr-spare-print-layout .spare-data-table-wrspare .c-item {
    ...
    function buildConsumeMetaPrintHtml(meta = {}) {
        const roInp = (type, val) => type === 'date'
            ? buildWrSpareDateUiPrintInput(val)
            : `<input type="${type}" class="wr-ro" value="${esc(val || '')}" readonly disabled>`;
        const fld = (label, inner, extraCls = '') =>
            `<div class="wr-maint-field${extraCls ? ' ' + extraCls : ''}"><label>${label}</label>${inner}</div>`;
        const pmsLabel = meta.pmsGroupNo ? safeTreeLabel(meta.pmsGroupNo) : '—';
        const jobBlock = renderConsumeWrPage2JobBlock(meta, { readonly: true, allowAdd: false });
        return `<section class="wr-maint-card wr-maint-body wr-file-no-anchor spare-consume-meta-form" aria-label="Consumption Report">
            <div class="wr-maint-grid wr-maint-grid-3">
                ${fld('File No.', roInp('text', meta.fileNo || ''))}
                ${fld('Voy. No.', roInp('text', meta.voyNo || ''))}
                ${fld('Place', roInp('text', meta.place || ''))}
    ...
    function consumeLogToPrintMeta(st, log) {
        const jobItems = consumeLogJobItems(log);
        const page1 = resolveConsumeLogPage1Meta(log, st);
        return {
            workDate: log.consumed_date || '',
            reportDate: log.made_on || '',
            ...
            jobItems,
            fileNo: page1.file_no || log.file_no || '',
            voyNo: page1.voy_no || log.voy_no || '',
            place: page1.place || log.place || '',
        };
    }

    function consumeLogToUsedParts(log) {
        return (log.lines || [])
    ...
    function buildConsumeReportUiPrintHtml(st, usedParts, meta, opts = {}) {
        const metaHtml = buildConsumeMetaPrintHtml(meta);
        const spareSection = `<div class="plan-layout spare-layout spare-req-work-layout spare-consume-work-layout wr-spare-work-layout wr-spare-full-width spare-consume-preview-layout wr-spare-print-layout">
            <main class="panel spare-main">
                <div id="wrSpareEditBlock">${buildWrSpareGroupHeaderPrintHtml(st, meta, usedParts)}</div>
                ${buildWrSpareUiTablePrintHtml(st, usedParts)}
            </main>
        </div>`;
        const inner = `${metaHtml}${spareSection}`;
        if (opts.innerOnly) return inner;
        return renderWrPrintShell('Consumption Report', '1', inner, 'repair', { hidePageTabs: true });
    }

    function enrichConsumeLogUsedParts(st, log) {
        return consumeLogToUsedParts(log).map(line => {
            const spare = (st.spares || []).find(s => s.id === line.spare_part_id);
            const c = spare ? TVC_SpareSchema.fromRow(spare) : {};
            return {
                spare_part_id: line.spare_part_id,
                part_no: c.makerPartNo || spare?.part_no || line.part_no || '—',
                name: c.name || spare?.name || line.name || '—',
                universal_code: c.universalItemCode || spare?.universal_code || '',
    ...
    function buildConsumeLogWrPrintHtml(st, log, opts = {}) {
        if (!log) return '';
        const type = consumeLogTypeMarker(log);
        const usedParts = enrichConsumeLogUsedParts(st, log);
        const printOpts = { ...opts, log };

        if (type.letter === 'M') {
            const rep = consumeLogLinkedReport(log, st);
            if (rep && rep.work_type !== 'POSTPONE' && TVC_App?.buildWorkReportPage2PrintHtmlFromReport) {
                const html = TVC_App.buildWorkReportPage2PrintHtmlFromReport(rep, st, usedParts, printOpts);
                if (html) return html;
            }
        }
        if (type.letter === 'D') {
            const dc = consumeLogLinkedDefect(log, st);
            if (dc && TVC_DefectReport?.buildDefectReportPage2PrintHtmlFromCase) {
                const html = TVC_DefectReport.buildDefectReportPage2PrintHtmlFromCase(dc, st, usedParts, printOpts);
                if (html) return html;
            }
        }

        const meta = consumeLogToPrintMeta(st, log);
        return buildConsumeReportUiPrintHtml(st, usedParts, meta, opts);
    }

    function buildConsumeLogWrPrintHtmlFromDraft(st, draft, opts = {}) {
        if (!draft) return '';
    ...
    async function renderConsumeModal() {
        ...
        if (m.consumeOpen && !isPreview) m.consumeLogListMode = true;
        ...
        const listMode = !!m.consumeLogListMode;
        const hasDisplayedLog = consumeLogListHasDisplayedLog(m);
        const listViewLocked = listMode && !m.consumeLogListEditing;
        ...
        const headTitle = isPreview ? 'Preview' : consumeReportWindowTitle(m);

        let headActions;
        if (isPreview) {
            headActions = `<button type="button" class="btn btn-sm" onclick="TVC_SpareMenu.consumePreviewOpenWorkReport()"${wrReportId ? '' : ' disabled'} title="${wrReportId ? 'View linked Work Report' : 'Not created from Work Report'}">Work Report</button>
            ...
            headActions = `<button type="button" id="consumeLogHeadNewBtn" class="btn btn-sm btn-green" onclick="TVC_SpareMenu.consumeLogNew()"${!canConsume || m.consumeLogListEditing ? ' disabled' : ''} title="${m.consumeLogListEditing ? 'Finish editing first' : (!canConsume ? 'No permission' : 'New consumption record')}">New</button>
               <button type="button" id="consumeLogHeadModifyBtn" class="btn btn-sm" onclick="TVC_SpareMenu.consumeLogEnterListEdit()"${!hasDisplayedLog || m.consumeLogListEditing ? ' disabled' : ''} title="${hasDisplayedLog ? 'Enable editing' : 'Select a log from the list first'}">Modify</button>
               <button type="button" id="consumeLogSaveBtn" class="btn btn-sm btn-green" onclick="TVC_SpareMenu.saveConsume()"${listViewLocked ? ' disabled' : ''}>Save</button>
               <button type="button" id="consumeLogCancelBtn" class="btn btn-sm" onclick="TVC_SpareMenu.consumeLogCancelEdit()"${listViewLocked ? ' disabled' : ''}>Cancel</button>
               <span class="orig-toolbar-sep" aria-hidden="true"></span>
               <button type="button" class="btn btn-sm" onclick="TVC_SpareMenu.consumeLogPrintPreview()"${!hasDisplayedLog ? ' disabled' : ''} title="${hasDisplayedLog ? 'Print consumption log' : 'Select a log from the list first'}">Print</button>
               <button type="button" class="btn btn-sm" onclick="TVC_SpareMenu.consumeLogWindowDocPreview()"${!hasDisplayedLog ? ' disabled' : ''} title="${hasDisplayedLog ? 'Preview consumption log' : 'Select a log from the list first'}">Preview</button>
               <span class="orig-toolbar-sep" aria-hidden="true"></span>
               <button type="button" class="btn btn-sm" onclick="TVC_SpareMenu.closeConsumeModal()">Close</button>
               <button type="button" class="modal-x" onclick="TVC_SpareMenu.closeConsumeModal()" title="Close">×</button>`;
        }
        ...
        const listToolbar = isPreview ? '' : (() => {
            ...
            return `
              ...
              <div class="filter-bar spare-list-search-bar">
                ...
                <button type="button" class="search-clear-btn hidden" title="Clear search" aria-label="Clear search"
                    onclick="TVC_App.clearSearchField('consumeSearch')">×</button>
                </div>
                <label class="sr-check"><input type="checkbox" ${m.showLowOnly ? 'checked' : ''}
        ...
        <div class="spare-req-work-wrap">
          <div class="spare-req-work-head">
            <h3 class="spare-req-work-title"><span id="consumeReportHeadTitle">${esc(headTitle)}</span></h3>
            <span class="spare-req-work-head-spacer"></span>
            ${headActions}
          </div>
          <div class="spare-req-work-scroll">
          ${renderConsumeMetaHtml(draft, { readonly: isPreview, department: st.department, listMode })}
          <div class="${layoutCls}">
            <main class="panel spare-main">
              ${listToolbar}
              ${editBlock}
        ...
        if (!isPreview) {
            ...
            TVC_App.bindSearchClearInput?.('consumeSearch');
            TVC_App.updateSearchClearBtn?.('consumeSearch');
        }
        ...
        return TVC_Inventory.saveConsumeLog({
            ...
            reporter_role: draft.reporter_role
                || (user && TVC_RBAC.isHqAccount(user)
                    ...
                    : (user ? (TVC_RBAC.resolveUserRole(user) || user.role || '') : '')),
            sort1: draft.sort1 || '',
            sort2: draft.sort2 || '',
            job_detail: draft.job_detail || '',
    ...
    async function syncConsumeLogFromWorkReport({ report, job, usedParts, form, user, department }) {
        ...
        const reportForm = report.report_form || {};
        const draft = {
            ...
            file_no: String(form?.fileNo || reportForm.fileNo || '').trim(),
    ...
    async function syncConsumeLogFromDefectReport({ defectCase, usedParts, user, department }) {
        ...
        const draft = {
            ...
            voy_no: String(defectCase.voy_no || '').trim(),
            place: String(defectCase.place || '').trim(),
    ...
    async function saveConsume() {
        ...
        if (draft.log_id) {
            ...
            const saved = await persistConsumeLogFromDraft(draft, {
                st, user, vesselId,
                extra: {
                    stock_applied_at: draft.stock_applied_at || prev?.stock_applied_at || '',
                    source: prev?.source || draft.source || 'consume',
                    work_report_id: prev?.work_report_id || draft.work_report_id || '',
                    defect_case_id: prev?.defect_case_id || draft.defect_case_id || '',
                },
            });
            draft.log_id = saved.id;
            draft.list_status = spareListStatus(draft);
            const approvalPrefix = isConsumedLogWindow(m) ? 'consumeLog' : 'consume';
            if (applySpareApprovalFromUi(draft, approvalPrefix, st.department)) {
            ...
            if (isConsumedLogWindow(m)) {
                m.consumeLogListEditing = false;
                applyConsumeLogSelection(m, saved.id);
                await loadConsumeLogIntoListWindow(saved.id, { preserveHistPopover: true });
                await refreshConsumeLogListUi();
            } else {
        ...
        try {
            ...
            m.consumeLastSavedLogId = saved.id;
            m.selectedConsumeLogId = saved.id;
            if (applySpareApprovalFromUi(draft, 'consumeLog', st.department)) {
                await persistConsumeLogFromDraft(draft, { st, user, vesselId });
            }
            await reloadSparesCache();
            await refresh();
    ...
    // ── Spare Parts Requisition — received qty input (requisition workflow) ──
    ...
    async function renderReceiveMetaHtml(draft) {
        ...
        return `<section class="spare-consume-meta-form spare-receive-meta-form" aria-label="Spare Parts Requisition receipt information">
    ...
    async function renderReceiveModal() {
        ...
        <div class="spare-req-work-wrap">
          <div class="spare-req-work-head">
            <h3 class="spare-req-work-title">Spare Parts Requisition
          ...
          <div class="plan-layout spare-layout spare-req-work-layout spare-receive-work-layout wr-spare-full-width">
            <main class="panel spare-main">
              ...
              <div class="filter-bar spare-list-search-bar spare-receive-search-bar">
                ...
                <button type="button" class="search-clear-btn hidden" title="Clear search" aria-label="Clear search"
                    onclick="TVC_App.clearSearchField('receiveSearch')">×</button>
                </div>
        ...
        updateSpareModalTreeToggleUi('receive');
        refreshReceiveListUi();
        TVC_App.bindSearchClearInput?.('receiveSearch');
        TVC_App.updateSearchClearBtn?.('receiveSearch');
        requestAnimationFrame(() => {
    ...
    function openConsumeModal() { consumeLogNew(); }
    ...
    function renderTxModal() {
        ...
        const title = isConsume ? 'Consumption Report / Qty.' : 'Spare Parts Requisition / Qty.';
    ...
    // ── Spare Item History (Note / Consumption / Requisition) ─────────
    let _shSpareId = null;
    let _shTab = 'note';
    let _shEditing = false;
    let _shHistoryData = null;
    ...
    function spareItemHistoryBtnHtml() {
        return `<button type="button" class="btn btn-sm spare-item-hist-btn" onclick="TVC_SpareMenu.openSpareItemHistoryForFocus()" title="View spare note, consumption / requisition history" disabled>📋 Spare History</button>`;
    }
    ...
    function syncSpareItemHistoryBtns() {
        ...
        document.querySelectorAll('.spare-item-hist-btn').forEach(btn => {
            btn.disabled = !id;
            btn.title = id
                ? 'View spare note, consumption / requisition history'
    ...
    function setSpareItemHistoryTab(tab) {
        if (tab === 'received') tab = 'consumed';
        if (tab !== 'note') _shEditing = false;
        _shTab = tab;
        rerenderSpareItemHistoryModal();
    }
    ...
    async function enterSpareItemNoteEdit() {
        const st = getState();
        if (!canModifySpare(st)) {
            await TVC_Dialog.alert(spareItemNoteEditDeniedMessage());
            return;
        }
        _shEditing = true;
        rerenderSpareItemHistoryModal();
        requestAnimationFrame(() => document.getElementById('shNoteInput')?.focus());
    }

    function cancelSpareItemNoteEdit() {
        _shEditing = false;
        rerenderSpareItemHistoryModal();
    }

    async function saveSpareItemNote() {
        const st = getState();
        if (!_shSpareId) return;
        if (!canModifySpare(st)) {
            await TVC_Dialog.alert(spareItemNoteEditDeniedMessage());
            return;
        }
        const text = String(document.getElementById('shNoteInput')?.value ?? '').trim();
        if (typeof TVC_SpareMeta !== 'undefined') {
            TVC_SpareMeta.setNote(_shSpareId, text);
        }
        _shEditing = false;
        rerenderSpareItemHistoryModal();
        await TVC_Dialog.success('Note saved.');
    }

    async function uploadSpareItemNoteAttachment() {
        const st = getState();
        if (!canModifySpare(st) || _shTab !== 'note' || !_shSpareId) return;
        const input = document.getElementById('shNoteAttachInput');
        if (!input?.files?.length || typeof TVC_SpareMeta === 'undefined') return;
        for (const f of input.files) await TVC_SpareMeta.addAttachment(_shSpareId, f);
        input.value = '';
        rerenderSpareItemHistoryModal();
    }

    function removeSpareItemNoteAttachment(attachmentId) {
        const st = getState();
        if (!canModifySpare(st) || _shTab !== 'note' || !_shSpareId) return;
        if (typeof TVC_SpareMeta === 'undefined') return;
        TVC_SpareMeta.removeAttachment(_shSpareId, attachmentId);
        rerenderSpareItemHistoryModal();
    }
    ...
    function paintSpareItemHistoryModal(host, st, spare, data) {
        if (_shTab === 'received') _shTab = 'consumed';
        const noteActive = _shTab === 'note' ? ' active' : '';
        const consumedActive = _shTab === 'consumed' ? ' active' : '';
        const reqActive = _shTab === 'requisition' ? ' active' : '';
        const meta = (typeof TVC_SpareMeta !== 'undefined') ? TVC_SpareMeta.get(_shSpareId) : { note: '', attachments: [] };

        let tabContent = '';
        if (_shTab === 'note') {
            const noteText = meta.note || '';
            const editing = !!_shEditing;
            const noteField = editing
                ? `<textarea id="shNoteInput" class="wp-proc-input" rows="12" aria-label="Spare Note">${esc(noteText)}</textarea>`
                : `<div class="proc-box wp-proc-box">${esc(noteText || 'No note registered.')}</div>`;
            tabContent = `
                <label class="wp-label">Note</label>
                ${noteField}`;
        } else if (_shTab === 'consumed') {
        ...
        const canEditNote = canModifySpare(st);
        const editingNote = _shTab === 'note' && !!_shEditing;
        const noteEditTip = escAttr(spareItemNoteEditDeniedMessage());
        const canAttach = canEditNote && _shTab === 'note';
        const attachTip = escAttr(canAttach
            ? 'Attach files to this spare note'
            : spareItemNoteEditDeniedMessage());
        let noteEditBtns = '';
        if (_shTab === 'note') {
            noteEditBtns = editingNote
                ? `<button type="button" class="btn btn-green" onclick="TVC_SpareMenu.saveSpareItemNote()">Save</button>
                <button type="button" class="btn" onclick="TVC_SpareMenu.cancelSpareItemNoteEdit()">Cancel</button>`
                : `<button type="button" class="btn" onclick="TVC_SpareMenu.enterSpareItemNoteEdit()"${canEditNote ? '' : ` disabled title="${noteEditTip}"`}>Modify</button>`;
        }
        const attachListHtml = renderSpareItemNoteAttachmentList(meta.attachments, canAttach);
        const leftActions = _shTab === 'note'
            ? `${noteEditBtns}<button type="button" class="btn" onclick="TVC_SpareMenu.closeSpareItemHistoryModal()">Close</button>`
            : `<button type="button" class="btn" onclick="TVC_SpareMenu.closeSpareItemHistoryModal()">Close</button>`;
        const attachPanelHtml = _shTab === 'note'
            ? `<div class="wp-modal-actions-right wp-attach-panel">
                    <div class="wr-attach-toolbar">
                        <button type="button" class="wr-attach-btn" onclick="document.getElementById('shNoteAttachInput').click()"${canAttach ? '' : ' disabled'} title="${attachTip}">📎 Attachment</button>
                        <input type="file" id="shNoteAttachInput" class="hidden" multiple onchange="TVC_SpareMenu.uploadSpareItemNoteAttachment()">
                    </div>
                    ${attachListHtml}
                </div>`
            : '';
        ...
        host.innerHTML = `
            ...
            <div class="wp-tabs">
                <button type="button" class="wp-tab${noteActive}" onclick="TVC_SpareMenu.setSpareItemHistoryTab('note')">Note</button>
            ...
            <div class="wp-tab-pane wp-tab-pane-${_shTab}">${tabContent}</div>
    ...
    function rerenderSpareItemHistoryModal() {
    ...
    async function renderSpareItemHistoryModal(forceReload) {
        const host = document.getElementById('spareItemHistoryBody');
        if (!host || !_shSpareId) return;
        if (_shTab === 'received') _shTab = 'consumed';
        const st = getState();
    ...
    async function openHistoryModal() {
        const st = getState();
        _spareHistCategory = spareHistNormalizeCategory(_spareHistCategory, st.user);
    ...
    async function setSpareHistCategory(key) {
        const st = getState();
        _spareHistCategory = spareHistNormalizeCategory(key, st.user);
        await renderSpareHistoryModal();
    }
    ...
    async function renderSpareHistoryModal() {
        ...
        const st = getState();
        const user = st.user;
        const isHq = !!(window.TVC_RBAC?.isHqAccount?.(user));
        const all = await listSpareDataXferHistory(120);
        const tabKeys = spareHistTabsForUser(user);
        const cat = spareHistNormalizeCategory(_spareHistCategory, user);
        _spareHistCategory = cat;
        const rows = all.filter(r => spareHistCategoryKey(r, { hq: isHq }) === cat);
    ...
    function equipmentListInput(id, value, extraClass = '') {
        const cls = ['spare-equipment-input', extraClass].filter(Boolean).join(' ');
        return `<input class="${cls}" id="${id}" value="${escAttr(String(value ?? ''))}">`;
    }

    function renderEquipmentListModal() {
        const st = getState();
        const m = modState(st);
        const groupLabel = equipmentListGroupLabel(st);
        const body = document.getElementById('spareEquipmentListBody');
        if (!body) return;
        if (!groupLabel) {
            body.innerHTML = '<p class="muted">Select a group in SPARE GROUP Tree.</p>';
            return;
        }
        const defs = equipmentDefsForGroup(st, groupLabel);
        const editing = m.equipmentListEditKey;
        const draft = m.equipmentListDraft || {};
        const cell = (text) => esc(String(text || '').trim() || '—');
        let rows = '';
        defs.forEach(d => {
            const isEdit = editing === d.name;
            const eqDisp = d.equipmentNo ? String(d.equipmentNo).padStart(2, '0') : '—';
            if (isEdit) {
                rows += `<tr class="is-editing">
                    <td class="c-eqno">${equipmentListInput('eq_list_no', draft.equipmentNo ?? d.equipmentNo ?? '', 'eq-list-no')}</td>
                    <td>${equipmentListInput('eq_list_name', draft.name ?? d.name, 'eq-list-name')}</td>
                    <td>${equipmentListInput('eq_list_maker', draft.maker ?? d.maker)}</td>
                    <td>${equipmentListInput('eq_list_model', draft.modelType ?? d.modelType)}</td>
                    <td>${equipmentListInput('eq_list_capacity', draft.capacity ?? d.capacity)}</td>
                    <td>${equipmentListInput('eq_list_serial', draft.serialNo ?? d.serialNo)}</td>
                    <td class="c-num">${d.spareCount}</td>
                    <td class="c-act">
                        <button type="button" class="btn btn-sm btn-green" onclick="TVC_SpareMenu.saveEquipmentListItem()">💾</button>
                        <button type="button" class="btn btn-sm" onclick="TVC_SpareMenu.cancelEquipmentListEdit()">Cancel</button>
                    </td>
                </tr>`;
                return;
            }
            rows += `<tr>
                <td class="c-eqno">${eqDisp}</td>
                <td>${cell(d.name)}</td>
                <td>${cell(d.maker)}</td>
                <td>${cell(d.modelType)}</td>
                <td>${cell(d.capacity)}</td>
                <td>${cell(d.serialNo)}</td>
                <td class="c-num">${d.spareCount}</td>
                <td class="c-act">
                    <button type="button" class="btn btn-sm" onclick="TVC_SpareMenu.startEditEquipmentListItem('${escAttr(d.name)}')" title="Edit">✏️</button>
                    <button type="button" class="btn btn-sm btn-red" onclick="TVC_SpareMenu.deleteEquipmentListItem('${escAttr(d.name)}')" title="Delete">🗑</button>
                </td>
            </tr>`;
        });
        if (editing === '__new__') {
            const nextNo = draft.equipmentNo || nextEquipmentNoForGroup(st, groupLabel);
            rows += `<tr class="is-editing is-new">
                <td class="c-eqno">${equipmentListInput('eq_list_no', nextNo, 'eq-list-no')}</td>
                <td>${equipmentListInput('eq_list_name', draft.name || '', 'eq-list-name')}</td>
                <td>${equipmentListInput('eq_list_maker', draft.maker || '')}</td>
                <td>${equipmentListInput('eq_list_model', draft.modelType || '')}</td>
                <td>${equipmentListInput('eq_list_capacity', draft.capacity || '')}</td>
                <td>${equipmentListInput('eq_list_serial', draft.serialNo || '')}</td>
                <td class="c-num">—</td>
                <td class="c-act">
                    <button type="button" class="btn btn-sm btn-green" onclick="TVC_SpareMenu.saveEquipmentListItem()">💾</button>
                    <button type="button" class="btn btn-sm" onclick="TVC_SpareMenu.cancelEquipmentListEdit()">Cancel</button>
        ...
        if (!rows) {
            rows = '<tr><td colspan="8" class="empty-row muted">No Equipment registered. Click Add Equipment below.</td></tr>';
        }
        body.innerHTML = `
            <h3 class="spare-sync-title">Equipment List</h3>
            <p class="spare-equipment-list-sub">${esc(safeTreeLabel(groupLabel))}</p>
            <p class="spare-sync-hint muted">EQ No. is the middle segment of Code (GG-<b>EE</b>-III). Example: MAIN BEARING = 01 → codes 01-01-001, 01-01-002 … Leave Maker~Serial blank to use group default.</p>
            <div class="spare-equipment-list-wrap sheet-scroll-original">
                <table class="data-table spare-equipment-list-table">
                    <thead><tr>
                        <th>EQ No.</th><th>Equipment</th><th>Maker</th><th>Model / Type</th><th>Capacity</th><th>Serial No.</th><th>Spares</th><th></th>
                    </tr></thead>
                    <tbody>${rows}</tbody>
                </table>
            </div>
            <div class="modal-actions spare-equipment-list-actions">
                <button type="button" class="btn btn-sm" onclick="TVC_SpareMenu.startAddEquipmentListItem()"${editing ? ' disabled' : ''}>➕ Add Equipment</button>
                <button type="button" class="btn" onclick="TVC_SpareMenu.closeEquipmentListModal()">Close</button>
            </div>`;
    }
    ...
    function startAddEquipmentListItem() {
        const st = getState();
        const m = modState(st);
        m.equipmentListEditKey = '__new__';
        m.equipmentListDraft = { equipmentNo: nextEquipmentNoForGroup(st, equipmentListGroupLabel(st)), name: '', maker: '', modelType: '', capacity: '', serialNo: '', oldName: '' };
        renderEquipmentListModal();
        setTimeout(() => document.getElementById('eq_list_name')?.focus(), 0);
    }

    function startEditEquipmentListItem(name) {
        const st = getState();
        const groupLabel = equipmentListGroupLabel(st);
        const d = equipmentDefsForGroup(st, groupLabel).find(x => x.name === name);
        if (!d) return;
        const m = modState(st);
        m.equipmentListEditKey = name;
        m.equipmentListDraft = {
            oldName: name,
    ...
    function cancelEquipmentListEdit() {
        const st = getState();
        const m = modState(st);
        m.equipmentListEditKey = null;
        m.equipmentListDraft = null;
    ...
    async function saveEquipmentListItem() {
        const st = getState();
        const m = modState(st);
        if (!canEditGroupHeader(st)) {
            await TVC_Dialog.alert('Chief Engineer / Captain permission required.');
        ...
        const g = (id) => { const el = document.getElementById(id); return el ? el.value.trim() : ''; };
        const oldName = String(m.equipmentListDraft?.oldName || m.equipmentListEditKey || '').trim();
        const name = g('eq_list_name');
        const maker = g('eq_list_maker');
        const modelType = g('eq_list_model');
        const capacity = g('eq_list_capacity');
        const serialNo = g('eq_list_serial');
        let equipmentNo = parseInt(g('eq_list_no'), 10);
        if (!Number.isFinite(equipmentNo) || equipmentNo < 1) {
            equipmentNo = m.equipmentListEditKey === '__new__'
                ? nextEquipmentNoForGroup(st, groupLabel)
                : equipmentNoForName(st, groupLabel, oldName);
        }
        equipmentNo = Math.min(99, Math.max(1, equipmentNo));
        if (!name) {
            await TVC_Dialog.alert('Enter Equipment name.');
            return;
        }
        const names = equipmentNamesForGroup(st, groupLabel);
        if (name !== oldName && names.some(n => n.toLowerCase() === name.toLowerCase())) {
            await TVC_Dialog.alert(`Equipment “${name}” already exists in this group.`);
            return;
        }
        const others = equipmentDefsForGroup(st, groupLabel).filter(d => {
            if (m.equipmentListEditKey === '__new__') return true;
            return d.name !== oldName;
        });
        const taken = others.find(d => d.equipmentNo === equipmentNo);
        if (taken) {
            await TVC_Dialog.alert(`EQ No. ${String(equipmentNo).padStart(2, '0')} is already used by “${taken.name}”.`);
            return;
        }
        const dept = st.department || groupSelectedNode(st)?.department || '';
        try {
            if (oldName && oldName !== name && oldName !== '__new__') {
                const spares = sparesForEquipment(st, groupLabel, oldName);
                if (spares.length) {
                    const rows = spares.map(s => TVC_SpareSchema.toRow({
                        ...canon(s),
                        equipment: name,
                        equipmentNo,
                    }));
                    if (typeof TVC_DB?.bulkPut === 'function') await TVC_DB.bulkPut('spare_parts', rows);
                        ...
                        const u = spareInventoryUser(st);
                        for (const r of rows) await TVC_Inventory.saveSpare(u, r);
                    }
                }
                const oldDef = findEquipmentGroupDef(st, groupLabel, oldName);
                if (oldDef?.id) {
                    await TVC_DB.del('maintenance_groups', oldDef.id);
                    st.groups = (st.groups || []).filter(gr => gr.id !== oldDef.id);
                }
            }
            const hasHeader = !!(maker || modelType || capacity || serialNo);
            const headerPayload = {
                department: dept,
                group: groupLabel,
                item_sort1: name,
            ...
            if (hasHeader) {
                await saveJobEquipmentHeader(spareInventoryUser(st), headerPayload);
            } else {
                const existing = findEquipmentGroupDef(st, groupLabel, name);
                const row = existing || {
                    id: 'grp-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8),
                    department: dept,
                    label: groupLabel,
                    sort_order: equipmentNo,
                    created_at: new Date().toISOString(),
                };
                const saved = {
                    ...row,
                ...
                await TVC_DB.put('maintenance_groups', saved);
                const groups = st.groups || [];
                const gi = groups.findIndex(gr => gr.id === saved.id);
                if (gi >= 0) groups[gi] = saved;
    ...
    async function deleteEquipmentListItem(name) {
        ...
        if (!groupLabel || !item) return;
        const spares = sparesForEquipment(st, groupLabel, item);
        const msg = spares.length
            ? `Delete Equipment “${item}”?\n\n${spares.length} spare item(s) will lose this Equipment tag (group default header will apply).`
            : `Delete Equipment “${item}”?`;
        if (!await TVC_Dialog.confirm({ message: msg })) return;
        try {
            const def = findEquipmentGroupDef(st, groupLabel, item);
            if (def?.id) {
                await TVC_DB.del('maintenance_groups', def.id);
                st.groups = (st.groups || []).filter(gr => gr.id !== def.id);
            }
            if (spares.length) {
                const rows = spares.map(s => TVC_SpareSchema.toRow({ ...canon(s), equipment: '', equipmentNo: 0 }));
                if (typeof TVC_DB?.bulkPut === 'function') await TVC_DB.bulkPut('spare_parts', rows);
                else {
                    const u = spareInventoryUser(st);
                    for (const r of rows) await TVC_Inventory.saveSpare(u, r);
            ...
            const m = modState(st);
            if (m.equipmentListEditKey === item) {
                m.equipmentListEditKey = null;
                m.equipmentListDraft = null;
            }
            renderEquipmentListModal();
    ...
    async function saveJobEquipmentHeader(user, { department, group, item_sort1, equipment_no, maker, modelType, capacity, serialNo }) {
        if (!window.TVC_DB) return;
    ...
    return {
        ...
        viewConsumedLog, openConsumedLogWindow, openConsumeLogModal, closeConsumeLogModal,
        consumeLogNew, consumeLogModify, consumePreviewModify, consumePreviewOpenWorkReport, cleanupConsumeWorkReportOverlay,
        consumeLogDelete, consumeLogPreview, consumeLogDetailReport, consumeLogReportConfirm, consumeLogReportApprove, consumeLogDocPreview, consumeLogPrint,
        consumeLogPrintPreview, consumeLogWindowDocPreview, consumeLogOpenList,
