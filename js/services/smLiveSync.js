/** THE VESSEL CODE — SM / Admin live sync (shared cloud DB → IndexedDB) */
const TVC_SmLiveSync = (function () {
    const POLL_MS = 30000;
    ...
    async function pollRegistry() {
        if (!user || !TVC_RBAC.isSuperSmAccount?.(user)) return false;
        ...
        try {
            ...
            console.warn('[TVC_SmLiveSync] registry', e);
            return false;
        }
    }
    ...
    function start(u, hooks = {}) {
        stop();
        if (!u || !TVC_RBAC.isSmAccount(u)) return;
        user = u;
...
if (typeof window !== 'undefined') window.TVC_SmLiveSync = TVC_SmLiveSync;
